...
MTD_VERSION = upstream
MTD_SITE = $(DLINK_GIT_STORAGE)/mtd-utils
MTD_LICENSE = GPL-2.0
MTD_LICENSE_FILES = COPYING
MTD_INSTALL_STAGING = YES

define MTD_RUN_AUTOGEN
	cd $(@D) && PATH=$(BR_PATH) ./autogen.sh
endef
MTD_PRE_CONFIGURE_HOOKS += MTD_RUN_AUTOGEN
HOST_MTD_PRE_CONFIGURE_HOOKS += MTD_RUN_AUTOGEN

ifeq ($(BR2_PACKAGE_MTD_JFFS_UTILS),y)
MTD_DEPENDENCIES += zlib lzo host-pkgconf
MTD_CONF_OPT += --with-jffs
else
MTD_CONF_OPT += --without-jffs
endif

ifeq ($(BR2_PACKAGE_MTD_UBIFS_UTILS),y)
MTD_DEPENDENCIES += util-linux zlib lzo host-pkgconf
MTD_CONF_OPT += --with-ubifs
ifeq ($(BR2_PACKAGE_OPENSSL),y)
MTD_DEPENDENCIES += openssl
MTD_CONF_OPT += --with-crypto
else
MTD_CONF_OPT += --without-crypto
endif
ifeq ($(BR2_PACKAGE_ZSTD),y)
MTD_DEPENDENCIES += zstd
MTD_CONF_OPT += --with-zstd
else
MTD_CONF_OPT += --without-zstd
endif
else
MTD_CONF_OPT += --without-ubifs
endif

ifeq ($(BR2_PACKAGE_MTD_TESTS),y)
MTD_CONF_OPT += --enable-tests --enable-install-tests
else
MTD_CONF_OPT += --disable-tests --disable-install-tests
endif
...
MTD_CONF_OPT += --with-xattr
else
MTD_CONF_OPT += --without-xattr
endif
...
MKFS_UBI = $(HOST_DIR)/usr/sbin/mkfs.ubifs
