...
#define DRU_NF_CONN_HAS_SKIP_OFFLOADS
	/*
	 * farisey: #4027
	 * support avoiding any offloads
	 * to use ipt_TRIGGER rules.
	 */
	int dru_skip_offloads;
#endif
...
extern int nf_ct_refresh_related(struct nf_conn *master, unsigned long extra_jiffies);
...
extern unsigned int nf_conntrack_count_sum;	//ibarinov
extern unsigned int nf_conntrack_period_peak;	//ibarinov
#endif

#if IS_ENABLED(CONFIG_NF_CONNTRACK_SIP) && \
    defined(CONFIG_DRU_THINOUT_HWNAT) && \
    IS_ENABLED(CONFIG_RA_HW_NAT) && \
    defined(CONFIG_NF_CONNTRACK_MARK)
#define SIP_MAY_BYPASS_HWNAT
extern unsigned int nf_conntrack_sip_bypass_hwnat;
#endif
...
#if IS_ENABLED(CONFIG_NETFILTER_XT_MATCH_URL)
extern unsigned int xt_url_loaded;
#else
#define xt_url_loaded	(0)
#endif
