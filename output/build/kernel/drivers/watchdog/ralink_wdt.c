...
/*
 * farisey:
 * predefined timeout is .. sorry Steven!
 * but is crap !
 */
//#define WATCHDOG_TIMEOUT 10		/* 10 sec default timeout */
static int WATCHDOG_TIMEOUT = 10; 	/* 10 sec default timeout */
...
static void DlinkSetupWtd(void)
{
...
#endif
{
	...
	switch (cmd) {
		...
		case WDIOC_SETTIMEOUT:
			if (copy_from_user(&WATCHDOG_TIMEOUT, (int*)arg, sizeof(WATCHDOG_TIMEOUT)) < 0)
				return -EFAULT;
			pr_debug("WatchDog Timer is set to %d sec.", WATCHDOG_TIMEOUT);
			DlinkSetupWtd();
			RaWdgReload();
			return 0;
...
static int __init watchdog_init(void)
{
	...
	pr_info("Ralink APSoC Hardware Watchdog Timer");
