...
)
{
	...
	if(likely(skb != NULL))
	{
		...
		pDataPhy = dma_map_single(NULL, (unsigned long)(skb->data), skb->len + addedLen, PCI_DMA_FROMDEVICE);		
...
#ifdef WORKQUEUE_BH
	cancel_work_sync(&mtk_interrupt_BH_result_wq);
#else
	tasklet_kill(&mtk_interrupt_BH_result_tsk);
#endif // WORKQUEUE_BH //

	//function pointer uninit
	ipsec_packet_put = NULL;
	ipsec_packet_get = NULL;
	ipsec_eip93CmdResCnt_check = NULL;
	ipsec_preComputeIn_cmdDescp_set = NULL;
	ipsec_preComputeOut_cmdDescp_set = NULL;
	ipsec_cmdHandler_cmdDescp_set = NULL;
	ipsec_espNextHeader_set = NULL;
	ipsec_espNextHeader_get = NULL;
	ipsec_pktLength_get = NULL;
	ipsec_eip93HashFinal_get = NULL;
	ipsec_eip93UserId_get = NULL;
#if !defined (CONFIG_HWCRYPTO_MEMPOOL)	
	ipsec_addrsDigestPreCompute_free = NULL;
	ipsec_cmdHandler_free = NULL;
#endif	
	ipsec_hashDigests_get = NULL;
	ipsec_hashDigests_set = NULL;
	ipsec_espSeqNum_get = NULL;

	ipsec_acc_enable = false;
	return;
}
