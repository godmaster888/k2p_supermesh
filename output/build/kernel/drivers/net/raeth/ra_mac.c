...
extern unsigned int wan_bcast_rx;
extern unsigned int wan_bcast_tx;
extern unsigned int wan_mcast_tx;
extern unsigned int wan_pause_rx;
extern unsigned int lan_pause_rx;
extern unsigned int wan_packets64b;
extern unsigned int wan_fragments;
extern unsigned int lan_fragments;
...
extern unsigned int __M2Q_table_raw(unsigned x);
...
int QDMARead(struct seq_file *seq, void *v)
{
	...
	for (i = 0; i < 64; i+=8){
		seq_printf(seq, " (%d,%d)(%d,%d)(%d,%d)(%d,%d)(%d,%d)(%d,%d)(%d,%d)(%d,%d)\n",
				i, __M2Q_table_raw(i), i+1, __M2Q_table_raw(i+1), i+2, __M2Q_table_raw(i+2), i+3, __M2Q_table_raw(i+3),
				i+4, __M2Q_table_raw(i+4), i+5, __M2Q_table_raw(i+5), i+6, __M2Q_table_raw(i+6), i+7, __M2Q_table_raw(i+7));
	}
...
/* START D-Link Patch #20672 [Ivan Barinov (August 2017)] - Kernel SLA Counters */
#ifdef CONFIG_DRU_SLA_COUNTERS
static struct proc_dir_entry *procWanTxBcast;
static struct proc_dir_entry *procWanRxBcast;
static struct proc_dir_entry *procWanTxMcast;
static struct proc_dir_entry *procWanPackets64b;
static struct proc_dir_entry *procWanFragments;
static struct proc_dir_entry *procLanFragments;
static struct proc_dir_entry *procWanPause;
static struct proc_dir_entry *procLanPause;
...
static int wtxb_read(struct seq_file *s, void *v)
{
	seq_printf(s, "%i\n", wan_bcast_tx);
	return 0;
}

static int wtxb_write(struct file *file, const char *buffer,
			unsigned long len, void *data)
{
	return len;
}

static int wtxb_single_open(struct inode *inode, struct file *file)
{
	return single_open(file, wtxb_read, NULL);
}

static ssize_t wtxb_single_write(struct file *file,
		const char __user *userbuf, size_t count, loff_t *off)
{
	return wtxb_write(file, userbuf, count, off);
}

static const struct file_operations wtxb_proc_fops = {
	.open      = wtxb_single_open,
	.write     = wtxb_single_write,
	.read      = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
...
static int wrxb_read(struct seq_file *s, void *v)
{
	seq_printf(s, "%i\n", wan_bcast_rx);
	return 0;
}

static int wrxb_write(struct file *file, const char *buffer,
			unsigned long len, void *data)
{
	return len;
}

static int wrxb_single_open(struct inode *inode, struct file *file)
{
	return single_open(file, wrxb_read, NULL);
}

static ssize_t wrxb_single_write(struct file *file,
		const char __user *userbuf, size_t count, loff_t *off)
{
	return wrxb_write(file, userbuf, count, off);
}

static const struct file_operations wrxb_proc_fops = {
	.open      = wrxb_single_open,
	.write     = wrxb_single_write,
	.read      = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
...
static int wtxm_read(struct seq_file *s, void *v)
{
	seq_printf(s, "%i\n", wan_mcast_tx);
	return 0;
}

static int wtxm_write(struct file *file, const char *buffer,
			unsigned long len, void *data)
{
	return len;
}

static int wtxm_single_open(struct inode *inode, struct file *file)
{
	return single_open(file, wtxm_read, NULL);
}

static ssize_t wtxm_single_write(struct file *file,
		const char __user *userbuf, size_t count, loff_t *off)
{
	return wtxm_write(file, userbuf, count, off);
}

static const struct file_operations wtxm_proc_fops = {
	.open      = wtxm_single_open,
	.write     = wtxm_single_write,
	.read      = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
...
static int wp64_read(struct seq_file *s, void *v)
{
	seq_printf(s, "%i\n", wan_packets64b);
	return 0;
}

static int wp64_write(struct file *file, const char *buffer,
			unsigned long len, void *data)
{
	return len;
}

static int wp64_single_open(struct inode *inode, struct file *file)
{
	return single_open(file, wp64_read, NULL);
}

static ssize_t wp64_single_write(struct file *file,
		const char __user *userbuf, size_t count, loff_t *off)
{
	return wp64_write(file, userbuf, count, off);
}

static const struct file_operations wp64_proc_fops = {
	.open      = wp64_single_open,
	.write     = wp64_single_write,
	.read      = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
...
static int wfr_read(struct seq_file *s, void *v)
{
	seq_printf(s, "%i\n", wan_fragments);
	return 0;
}

static int wfr_write(struct file *file, const char *buffer,
			unsigned long len, void *data)
{
	return len;
}

static int wfr_single_open(struct inode *inode, struct file *file)
{
	return single_open(file, wfr_read, NULL);
}

static ssize_t wfr_single_write(struct file *file,
		const char __user *userbuf, size_t count, loff_t *off)
{
	return wfr_write(file, userbuf, count, off);
}

static const struct file_operations wfr_proc_fops = {
	.open      = wfr_single_open,
	.write     = wfr_single_write,
	.read      = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
...
static int lfr_read(struct seq_file *s, void *v)
{
	seq_printf(s, "%i\n", lan_fragments);
	return 0;
}

static int lfr_write(struct file *file, const char *buffer,
			unsigned long len, void *data)
{
	return len;
}

static int lfr_single_open(struct inode *inode, struct file *file)
{
	return single_open(file, lfr_read, NULL);
}

static ssize_t lfr_single_write(struct file *file,
		const char __user *userbuf, size_t count, loff_t *off)
{
	return lfr_write(file, userbuf, count, off);
}

static const struct file_operations lfr_proc_fops = {
	.open      = lfr_single_open,
	.write     = lfr_single_write,
	.read      = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
...
static int wrxp_read(struct seq_file *s, void *v)
{
	seq_printf(s, "%i\n", wan_pause_rx);
	return 0;
}

static int wrxp_write(struct file *file, const char *buffer,
			unsigned long len, void *data)
{
	return len;
}

static int wrxp_single_open(struct inode *inode, struct file *file)
{
	return single_open(file, wrxp_read, NULL);
}

static ssize_t wrxp_single_write(struct file *file,
		const char __user *userbuf, size_t count, loff_t *off)
{
	return wrxp_write(file, userbuf, count, off);
}

static const struct file_operations wrxp_proc_fops = {
	.open      = wrxp_single_open,
	.write     = wrxp_single_write,
	.read      = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
...
static int lrxp_read(struct seq_file *s, void *v)
{
	seq_printf(s, "%i\n", lan_pause_rx);
	return 0;
}

static int lrxp_write(struct file *file, const char *buffer,
			unsigned long len, void *data)
{
	return len;
}

static int lrxp_single_open(struct inode *inode, struct file *file)
{
	return single_open(file, lrxp_read, NULL);
}

static ssize_t lrxp_single_write(struct file *file,
		const char __user *userbuf, size_t count, loff_t *off)
{
	return lrxp_write(file, userbuf, count, off);
}

static const struct file_operations lrxp_proc_fops = {
	.open      = lrxp_single_open,
	.write     = lrxp_single_write,
	.read      = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
...
/* START D-Link Patch #20672 [Ivan Barinov (August 2017)] - Kernel SLA Counters */
    ...
    if ((procWanTxBcast = create_proc_entry(PROCREG_WAN_TX_BCAST, 0, procRegDir)))
	    procWanTxBcast->proc_fops = &wtxb_proc_fops;
    else
#else
    if (!(procWanTxBcast = proc_create(PROCREG_WAN_TX_BCAST, 0, procRegDir, &wtxb_proc_fops)))
#endif
	    printk("!! FAIL to create %s PROC !!\n", PROCREG_WAN_TX_BCAST);

    // WAN RX Broadcast

#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
    if ((procWanRxBcast = create_proc_entry(PROCREG_WAN_RX_BCAST, 0, procRegDir)))
	    procWanRxBcast->proc_fops = &wrxb_proc_fops;
    else
#else
    if (!(procWanRxBcast = proc_create(PROCREG_WAN_RX_BCAST, 0, procRegDir, &wrxb_proc_fops)))
#endif
	    printk("!! FAIL to create %s PROC !!\n", PROCREG_WAN_RX_BCAST);

    // WAN TX Multicast

#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
    if ((procWanTxMcast = create_proc_entry(PROCREG_WAN_TX_MCAST, 0, procRegDir)))
	    procWanTxMcast->proc_fops = &wtxm_proc_fops;
    else
#else
    if (!(procWanTxMcast = proc_create(PROCREG_WAN_TX_MCAST, 0, procRegDir, &wtxm_proc_fops)))
#endif
	    printk("!! FAIL to create %s PROC !!\n", PROCREG_WAN_TX_MCAST);

    // Packets 64 Bytes

#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
    if ((procWanPackets64b = create_proc_entry(PROCREG_PACK64B, 0, procRegDir)))
	    procWanPackets64b->proc_fops = &wp64_proc_fops;
    else
#else
    if (!(procWanPackets64b = proc_create(PROCREG_PACK64B, 0, procRegDir, &wp64_proc_fops)))
#endif
	    printk("!! FAIL to create %s PROC !!\n", PROCREG_PACK64B);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
    if ((procWanFragments = create_proc_entry(PROCREG_WAN_FRAG, 0, procRegDir)))
	    procWanFragments->proc_fops = &wfr_proc_fops;
    else
#else
    if (!(procWanFragments = proc_create(PROCREG_WAN_FRAG, 0, procRegDir, &wfr_proc_fops)))
#endif
	    printk("!! FAIL to create %s PROC !!\n", PROCREG_WAN_FRAG);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
    if ((procLanFragments = create_proc_entry(PROCREG_LAN_FRAG, 0, procRegDir)))
	    procLanFragments->proc_fops = &lfr_proc_fops;
    else
#else
    if (!(procLanFragments = proc_create(PROCREG_LAN_FRAG, 0, procRegDir, &lfr_proc_fops)))
#endif
	    printk("!! FAIL to create %s PROC !!\n", PROCREG_LAN_FRAG);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
    if ((procWanPause = create_proc_entry(PROCREG_WAN_RX_PAUSE, 0, procRegDir)))
	    procWanPause->proc_fops = &wrxp_proc_fops;
    else
#else
    if (!(procWanPause = proc_create(PROCREG_WAN_RX_PAUSE, 0, procRegDir, &wrxp_proc_fops)))
#endif
	    printk("!! FAIL to create %s PROC !!\n", PROCREG_WAN_RX_PAUSE);

    // RX LAN Pause Frames

#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
    if ((procLanPause = create_proc_entry(PROCREG_LAN_RX_PAUSE, 0, procRegDir)))
	    procLanPause->proc_fops = &lrxp_proc_fops;
    else
#else
    if (!(procLanPause = proc_create(PROCREG_LAN_RX_PAUSE, 0, procRegDir, &lrxp_proc_fops)))
#endif
	    printk("!! FAIL to create %s PROC !!\n", PROCREG_LAN_RX_PAUSE);
...
#ifdef CONFIG_DRU_SLA_COUNTERS
if (procWanTxBcast)
    	remove_proc_entry(PROCREG_WAN_TX_BCAST, procRegDir);

if (procWanTxMcast)
    	remove_proc_entry(PROCREG_WAN_TX_MCAST, procRegDir);

if (procWanPackets64b)
    	remove_proc_entry(PROCREG_PACK64B, procRegDir);

if (procWanFragments)
    	remove_proc_entry(PROCREG_WAN_FRAG, procRegDir);

if (procLanFragments)
    	remove_proc_entry(PROCREG_LAN_FRAG, procRegDir);

if (procWanPause)
    	remove_proc_entry(PROCREG_WAN_RX_PAUSE, procRegDir);

if (procLanPause)
    	remove_proc_entry(PROCREG_LAN_RX_PAUSE, procRegDir);
#endif /* CONFIG_DRU_SLA_COUNTERS */
/* END D-Link Patch */
