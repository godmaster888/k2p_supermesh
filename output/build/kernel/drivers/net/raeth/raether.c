...
#include <linux/etherdevice.h>	/* is_multicast_ether_addr() */
    ...
    defined(CONFIG_NETLINK_MONITOR_WAN_LED_DIR_853) || \
    defined(CONFIG_NETLINK_MONITOR_WAN_LED_DAP_300P)
//#18026, #20550
...
/* START D-Link Patch #20672 [Ivan Barinov (August 2017)] - Kernel SLA Counters */
#ifdef CONFIG_DRU_SLA_COUNTERS
unsigned int wan_bcast_rx = 0;
unsigned int wan_bcast_tx = 0;
unsigned int wan_mcast_tx = 0;
unsigned int wan_pause_rx = 0;
unsigned int lan_pause_rx = 0;

unsigned int wan_packets64b = 0;
unsigned int wan_fragments = 0;
unsigned int lan_fragments = 0;

#if defined(CONFIG_DRU_RA_VLANS)
#define lan_vid	((unsigned short)dru_sysctl_data.ra_vlans.lan_vid[0])
#define wan_vid	((unsigned short)dru_sysctl_data.ra_vlans.wan_vid)
#else
//original
...
static unsigned int __M2Q_table[64] = {0};
unsigned int __M2Q_table_raw(unsigned x)
{
	return __M2Q_table[x];
}
unsigned int M2Q_table(const struct sk_buff *skb)
{
	return __M2Q_table[skb->mark & 0x3f]; // mark % 64
}
unsigned int lan_wan_separate = 0;

#if defined(CONFIG_HW_SFQ)
static unsigned int __web_sfq_enable = 0;
bool web_sfq_enable(const struct sk_buff *skb)
{
	return __web_sfq_enable && (skb->mark & 2);
}
EXPORT_SYMBOL(web_sfq_enable);
#endif

EXPORT_SYMBOL(__M2Q_table_raw);
...
void skb_dump(struct sk_buff* sk) {
        ...
        for(i=(unsigned int)sk->head;i<=(unsigned int)sk->data+32;i++) {
                if((i % 32) == 0)
                        printk("\n");
...
#if defined(CONFIG_DRU_FILTERING)
static unsigned long dru_total_frames = 0;
static unsigned long dru_dropped_frames = 0;

typedef u32 dru_parse_result_t;
/**
 * нулевой байт[7:0]:
 * [1:0] :	результат
 * 		00 - не смог распарсить
 * 		01 - распарсился
 * 		10 - битый пакет (размеры не те, например)
 * 		11 - reserved
 * [2]   :	признак 802.1q
 * 		0 - нету
 * 		1 - есть (vlan_hdr = data)
 * [4:3] :	сетевая часть
 * 		00 - неизвестый
 * 		01 - IP
 * 		10 - PPPoE Session
 * 		11 - reserved
 * [6:5] :	транспортная часть
 * 		00 - неизвестый
 * 		01 - TCP
 * 		10 - UDP
 * 		11 - reserved
 * [7]   :	reserved
 */
#define D_PARSE_RSLT_UNPARSED	0x0
#define D_PARSE_RSLT_PARSED	0x1
#define D_PARSE_RSLT_BROKEN	0x2
#define D_PARSE_GET_RESULT(x)	((x) & 0x3)

#define D_PARSE_RSLT_VLAN	(1 << 2)
#define D_PARSE_HAS_VLAN(x)	((x) & D_PARSE_RSLT_VLAN)

#define D_PARSE_RSLT_IP		(1 << 3)
#define D_PARSE_RSLT_PPP_SES	(2 << 3)
#define D_PARSE_LOW_PROTO(x)	((x) & 0x18)

#define D_PARSE_RSLT_TCP	(1 << 5)
#define D_PARSE_RSLT_UDP	(2 << 5)
#define D_PARSE_TRANS_PROTO(x)	((x) & 0x60)

/**
 * первый байт[15:8]:
 * смещение IP-загловка от skb->data
 * Сейчас не более 12 байт, но пусть будет запас.
 */
#define D_PARSE_SET_IP_OFFSET(x, off) ((x) |= ((off) << 8))
#define D_PARSE_GET_IP_OFFSET(x) (((x) >> 8) & 0xff)

/**
 * второй байт[23:16]:
 * смещение транспортного загловка от skb->data
 * По идее IP + (20 или 24) байта.
 * Тоже с запасом
 */
#define D_PARSE_SET_TRANS_OFFSET(x, off) ((x) |= ((off) << 16))
#define D_PARSE_GET_TRANS_OFFSET(x) (((x) >> 16) & 0xff)

static inline dru_parse_result_t dru_parse_skb(struct sk_buff *rx_skb, int *payload_size)
{
	dru_parse_result_t res = D_PARSE_RSLT_UNPARSED;
	struct ethhdr *eth = eth_hdr(rx_skb);//это у них вроде есть
	__be16 proto;
	unsigned ip_hd_offset = 0;
	struct iphdr *iph;
	struct udphdr *uh;
	struct tcphdr *th;
	int x;
	...
	/*
	 * farisey:
	 * по-хорошему, тут на каждом этапе надо проверок
	 * на размеры нахренячить, что заявленные sizes
	 * не вылезают за реальный размер фрейма.
	 * Но это просадит производительность,
	 * да и память вроде как выделяется суровыми кусками,
	 * так что нечего единицы байт считать.
	 */
	if (likely(eth->h_proto == __constant_htons(ETH_P_8021Q))) {
		ip_hd_offset += 4;//skip 802.1q header
		proto = *(__be16*)(rx_skb->data + 2);
		//proto = ((struct vlan_hdr*)rx_skb->data)->h_vlan_encapsulated_proto;
		res |= D_PARSE_RSLT_VLAN;
	}
	else proto = eth->h_proto;
	...
	switch (proto) {
		...
		case __constant_htons(ETH_P_PPP_SES):
			res |= D_PARSE_RSLT_PPP_SES;
			//проверим версию pppoe и code (session data)
			if (*(__be16*)(rx_skb->data + ip_hd_offset /*+ 1 ошибся?? вроде не нужно*/) != __constant_htons(0x1100)) return res;
			//проверим, что вложен IPv4
			if (*(__be16*)(rx_skb->data + ip_hd_offset + 6) != __constant_htons(0x0021)) return res;
			ip_hd_offset += 8;//сместим позицию IP заголовка
			break;
		default:
			return res;//неизвестый нам пакет. Пусть живёт.
	}

	D_PARSE_SET_IP_OFFSET(res, ip_hd_offset);
	iph = (struct iphdr*)(rx_skb->data + ip_hd_offset);
	//на всякий случай проверочку
	if (unlikely(rx_skb->len < ip_hd_offset + ntohs(iph->tot_len)))
		return (res | D_PARSE_RSLT_BROKEN);

	x = ip_hd_offset + (iph->ihl<<2);//размер: IP заголовок + что до него
	D_PARSE_SET_TRANS_OFFSET(res, x);//=> x - смещение транспортного заголовка
	x = rx_skb->len - x;//реальный размер тр.заголовка + payload

	switch (iph->protocol) {
		case IPPROTO_TCP:
			res |= D_PARSE_RSLT_TCP;
			th = (void*)iph + (iph->ihl<<2);
			x -= (th->doff << 2);//вычисленный размер пейлоада
			break;
		case IPPROTO_UDP:
			res |= D_PARSE_RSLT_UDP;
			uh = (void*)iph + (iph->ihl<<2);
			x -= sizeof(struct udphdr);//реальный размер пейлоада
			break;
		default:
			return res;
	}
	if (payload_size) *payload_size = x;

	return (res | D_PARSE_RSLT_PARSED);
}

#endif

#ifdef CONFIG_DRU_USE_AVENRUN_5SEC
#define AV_IDX		3
#else
#define AV_IDX		0
#endif
 ...
 * -1 - broken
 */
static inline int dlink_checks_to_drop_it(struct sk_buff *rx_skb, dru_parse_result_t parse_res, int payload_size)
{
#if 0
	int x;
	unsigned ip_hd_offset = 0;
	...
	/*
	 * Проверим загрузку.
	 * Здесь автоматом срабатывает случай, когда проверка
	 * на загрузку отключена. Реальная загрузка не может
	 * быль меньше нуля (unsigned) и условие точно
	 * не сработает.
	 * Правда, всё это фигня, т.к. загрузка считается
	 * за минуту, и всё это сильно волатильно.
	 */
	if ( (avenrun[AV_IDX] < dru_sysctl_data.udp_filter.cpuload_avenrun) &&
		(dru_sysstat.idle.percents >= dru_sysctl_data.udp_filter.idle)
	)
		return 0;//too low load. Do not filter.
#if 0
	/*
	 * после eth_type_trans заполнены
	 * rx_skb->dev - местное присвоение (в rt2880_eth_recv) не нужно.
	 * в некоторых случаях skb->pkt_type, возможно
	 * подразумевается, что по дефолту там 0 (PACKET_HOST - to us)?
	 * я вообще хуй пойму что тут уже заполнено, а что нет.
	 * Вот можно skb_(network|transport)_header юзать или нет???
	 * Массивы из skbuff, кто, куда, откуда... хз!
	 * UPDATE. Практика показала, что у них заполнен mac_header
	 * и data указывает на "после него". Network и transport
	 * не заполнены. А rx_skb->protocol вообще х знает что.
	 */
	eth = eth_hdr(rx_skb);//это у них вроде есть
#if 0
	see it in rt2880_eth_recv
	if (dru_sysctl_data.mc_fwd) {
		/* всё с мультикастовым dest mac пропускать.
		 * Надеюсь, у ралинка с маками проблем нет.
		 * Преобразованием mc->uc для воздуха вспоминается
		 * опять же.
		 */
	...
	if (likely(eth->h_proto == __constant_htons(ETH_P_8021Q))) {
#ifdef CONFIG_DRU_RA_LAN_VID
		struct vlan_hdr *vh = (struct vlan_hdr *)rx_skb->data;

		//принимать всё с лана, если такая опция включена
		if (dru_sysctl_data.udp_filter.allow_lan)
			if ( ra_is_lan_be(vh->h_vlan_TCI & __constant_htons(VLAN_VID_MASK)) )
				return 0;
#endif
		ip_hd_offset += 4;//skip 802.1q header
		proto = *(__be16*)(rx_skb->data + 2);
	...
	//data показывает на "после мак". Там должо быть минимум - IP + UDP
	//а это минимум 20 байт IP плюс 8 байт ровно UDP (это и без упаковки ровно).
	x = ip_hd_offset + 20 + sizeof(struct udphdr);//сколько байт минимум должно быть в len
	if (rx_skb->len < x) return 0;//слишком маленький для UDP. Либо битый.
	switch (proto) {
		case __constant_htons(ETH_P_IP):
			//всё готово тут.
			break;
		...
		case __constant_htons(ETH_P_PPP_SES):
			if (!dru_sysctl_data.udp_filter.pppoe) return 0;//фильтр pppoe отключён
			x += 8; //6 - pppoe + 2 - ptp
			if (rx_skb->len < x) return 0;//слишком маленький для UDP. Либо битый.
			//проверим версию pppoe и code (session data)
			if (*(__be16*)(rx_skb->data + ip_hd_offset + 1) != __constant_htons(0x1100)) return 0;
			//проверим, что вложен IPv4
			if (*(__be16*)(rx_skb->data + ip_hd_offset + 6) != __constant_htons(0x0021)) return 0;
			ip_hd_offset += 8;//сместим позицию IP заголовка
		default:
			return 0;//неизвестый нам пакет. Пусть живёт.
	}
	iph = (struct iphdr*)(rx_skb->data + ip_hd_offset);

	if (iph->protocol != IPPROTO_UDP) return 0;//не наш клиент
	uh = (void*)iph + (iph->ihl<<2);//прыгнем на UDP
	//проверим не убежал ли UDP-хидер за границы
	x = (unsigned char*)(uh + 1) - rx_skb->data;
	if (unlikely(x > rx_skb->len)) return -1;//UDP вылез за data
#endif
	if (D_PARSE_TRANS_PROTO(parse_res) != D_PARSE_RSLT_UDP)
		return 0;//не наш клиент
	uh = (struct udphdr*)(rx_skb->data + D_PARSE_GET_TRANS_OFFSET(parse_res));
	//белый список
#if 0
#define DBG_ACCEPT(txt) do {printk("dlink: accept %s\n", txt);} while (0)
#else
#define DBG_ACCEPT(txt) do { } while (0)
#endif
#if 0
	if ((uh->source == __constant_htons(53)) || (uh->dest == __constant_htons(53))) {
		DBG_ACCEPT("DNS");
		return 0;
	}
#endif
	//if ((uh->source < __constant_htons(1024)) || (uh->dest < __constant_htons(1024))) {
	//farisey: неравенства в реверсном относительно проца порядке байт даёт жопу.
	//равенство можно, там пох порядок
	if ((ntohs(uh->source) < 1024) || (ntohs(uh->dest) < 1024)) {
		//DBG_ACCEPT("IANA");
		return 0;
	}
	//логическое И. В l2tp src = dst = 1701.
	if ((uh->source == __constant_htons(1701)) && (uh->dest == __constant_htons(1701))) {
		//DBG_ACCEPT("L2TP");
		return 0;
	}
	if (uh->dest == __constant_htons(1900)) {
		DBG_ACCEPT("UPNP");
		return 0;
	}
	if ((uh->source == __constant_htons(5060)) || (uh->dest == __constant_htons(5060))) {
		DBG_ACCEPT("SIP");
		return 0;
	}
	if (uh->dest == __constant_htons(20014)) {
		//DBG_ACCEPT("WoT");
		return 0;
	}
	if (uh->dest == __constant_htons(8888)) {
		DBG_ACCEPT("Daemon");
		return 0;
	}
#if 0
	if ((uh->source == __constant_htons(67)) || (uh->source == __constant_htons(68))) {
		DBG_ACCEPT("DHCP");
		return 0;
	}
#endif
#if 0
	x = rx_skb->len - x;//размер пейлоада UDP
	if (x >= dru_sysctl_data.udp_filter.threshold) return 0;//крупные UDP пусть будут
	if (dru_sysctl_data.udp_filter.heuristic_utp) //поиск uTP
		if (x) //непустой пейлоад
			if (*(unsigned char*)(uh + 1) != 0x01) //не uTP
				return 0;//пусть живёт мелкий, но не uTP
#endif
	if (payload_size >= dru_sysctl_data.udp_filter.threshold) return 0;//крупные UDP пусть будут
	if (dru_sysctl_data.udp_filter.heuristic_utp) //поиск uTP
		if (payload_size > 0) //непустой пейлоад (параноидально проверим на положительность)
			if (*(unsigned char*)(uh + 1) != 0x01) //не uTP
				return 0;//пусть живёт мелкий, но не uTP
	return 1;//дропать мелочь
#if 0
		if ( !(cnt % DMP_DIV) ) {
			//каждый DMP_DIV задампим
			printk("++++ %u, proto=%d, len=%u, x=%d\n", cnt/DMP_DIV, ntohs(rx_skb->protocol), rx_skb->len, x);
			//dp = dmpbuf;
			//for (i = 0; (i < 10) && (i < rx_skb->len); i++, dp += 3)
			//	sprintf(dp, "%02x ", rx_skb->data[i]);
			//printk("++++ %u, %s\n", cnt/DMP_DIV, dmpbuf);
			for (i = 0; (i < 32) && (i < rx_skb->len); i++)
...
#if 0
		///cnt++;
		if ( x && likely(rx_skb->len >= x) ) {
			iph = (void*)rx_skb->data + (x - 20);//скипнем eth [+ 802.1q vlan]
			if (iph->protocol == IPPROTO_UDP) {
				uh = (void*)iph + (iph->ihl<<2);
				if (unlikely(rx_skb->len < (void*)(uh + 1) - (void*)rx_skb->data))
					goto drop_it;//слишком пакет мал для заявленных в хидерах размеров
				if ((uh->source == __constant_htons(53)) || (uh->dest == __constant_htons(53)))
					goto process_it;//принимать DNS
				if ((uh->source == __constant_htons(5060)) || (uh->dest == __constant_htons(5060)))
					goto process_it;//принимать SIP
				//реальный размер пейлоада
				x = rx_skb->len - ( (void*)(uh + 1) - (void*)rx_skb->data );
				if (x < 512) goto drop_it;
			}
			goto process_it;
		drop_it:
			skb_dump(rx_skb);
			ei_local->stat.rx_dropped++;//TODO PSEUDO_SUPPORT ?
			continue;//????? как реально дропнуть пакет???
		}
process_it:
#endif
}
/** end farisey */
#endif /* CONFIG_DRU_UDP_FILTER */
...
static inline int dlink_checks_cpuload_to_drop(void)
{
	//long unsigned int avg_prs;

	if (avenrun[AV_IDX] <= dru_sysctl_data.load_filter.cpuload_min_avenrun) return 0;//низкая нагрузка
	if (avenrun[AV_IDX] >= dru_sysctl_data.load_filter.cpuload_max_avenrun) return 1;//высокая нагрузка
	//avg_prs = 100 * LOAD_INT(avenrun[AV_IDX]) + LOAD_FRAC(avenrun[AV_IDX]);//загрузка в процентах
	//if (100 * dru_dropped_frames / dru_total_frames < 100 * (avg_prs - dru_sysctl_data.load_filter.cpuload_min) / (dru_sysctl_data.load_filter.cpuload_max - dru_sysctl_data.load_filter.cpuload_min))
	//1. переходим к avenrun без пересчётов в проценты, вроде можно в fixed point
	//2. умножаем пропорцию накрест.
	//3. надеюсь переполнения в умножении не будет.
	if (dru_dropped_frames * (dru_sysctl_data.load_filter.cpuload_max_avenrun - dru_sysctl_data.load_filter.cpuload_min_avenrun) <
	    dru_total_frames   * (avenrun[AV_IDX]                                 - dru_sysctl_data.load_filter.cpuload_min_avenrun))
		return 1;//мало проредили, надо больше
	return 0;
}
#endif
 ...
 * 1 - drop
 */
static inline int dlink_checks_cpuidle_to_drop(void)
{
	//long unsigned int avg_prs;

	if (dru_sysstat.idle.percents >= dru_sysctl_data.idle_filter.cpuidle_max) return 0;//низкая нагрузка
	if (dru_sysstat.idle.percents <= dru_sysctl_data.idle_filter.cpuidle_min) return 1;//высока нагрузка
	if (dru_dropped_frames * (dru_sysctl_data.idle_filter.cpuidle_max - dru_sysctl_data.idle_filter.cpuidle_min) <
	    dru_total_frames   * (dru_sysctl_data.idle_filter.cpuidle_max - dru_sysstat.idle.percents))
		return 1;//мало проредили, надо больше
	return 0;
}
#endif
...
/*
 * -1 - igmp from unclear side
 * 0 - is not igmp or definitely not from LAN
 * 1 - igmp from LAN
 */
static inline int dlink_is_igmp_from_lan(struct sk_buff *rx_skb)
{
	unsigned ip_hd_offset = 0;
	__be16 proto;
	struct ethhdr *eth = eth_hdr(rx_skb);
	struct iphdr *iph;
	...
	if (proto != __constant_htons(ETH_P_IP)) return 0; //not IGMP
	iph = (struct iphdr*)(rx_skb->data + ip_hd_offset);
	if (iph->protocol != IPPROTO_IGMP) return 0; //not IGMP
	if (likely(eth->h_proto == __constant_htons(ETH_P_8021Q))) return 1;
	return -1;//IGMP without vlan header. unknown source.
}
#endif
...
#endif
{
...
#if defined(CONFIG_DRU_FILTERING)
	dru_parse_result_t parse_res = 0;//avoid warning
	int payload_size = 0;//avoid warning
#endif
	...
	for ( ; ; ) {
		...
		/*
		if ( !(cnt % DMP_DIV) ) {
			//farisey: каждый DMP_DIV задампим
			printk("++++ %u\n", cnt/DMP_DIV);
		}
		*/
#ifdef CONFIG_RAETH_NAPI
...
/*TODO*/
		...
		if (is_multicast_ether_addr(eth_hdr(rx_skb)->h_dest)) {
#if defined(CONFIG_DRU_RAWIFI)
			if (dlink_is_igmp_from_lan(rx_skb)) {
				//дадим ресурсов для IGMP
				if (dru_wifi_throttle()) {
					/* нет смысла фильтровать то,
					 для чего только что придавили WiFi.
					 Сильно вряд, что mc_fwd будет выключен.
					 Так что сразу акцептим. */
					goto skip_mc_filtering;
				}
			}
#endif
			if (likely(dru_sysctl_data.mc_fwd)) {
				/*
				static unsigned long long mc_accepted = 0;
				if ( ((++mc_accepted) % 1024) == 0 )
					printk("+++ accepted %lluk mc frames\n", mc_accepted>>10);
				*/
				goto skip_mc_filtering;
			}
		}
		...
		if (likely(
#if defined(CONFIG_DRU_UDP_FILTER)
			dru_sysctl_data.udp_filter.enable ||
#endif
#if defined(CONFIG_DRU_IDLE_FILTER)
			dru_sysctl_data.idle_filter.enable ||
#endif
#if defined(CONFIG_DRU_LOAD_FILTER)
			dru_sysctl_data.load_filter.enable ||
#endif
			0)) {
			parse_res = dru_parse_skb(rx_skb, &payload_size);
			if (D_PARSE_GET_RESULT(parse_res) != D_PARSE_RSLT_PARSED)
				goto skip_mc_filtering;//нераспарсенные не трогаем
			if ( (D_PARSE_TRANS_PROTO(parse_res) == D_PARSE_RSLT_TCP) && (payload_size <= 0))
				goto skip_mc_filtering;//не трогаем служебные TCP
		}
#endif

#if defined(CONFIG_DRU_UDP_FILTER)
		if ( dru_sysctl_data.udp_filter.enable ) {
			int drop_res = dlink_checks_to_drop_it(rx_skb, parse_res, payload_size);
#if defined(CONFIG_DRU_UDP_FILTER_STAT)
			switch (drop_res) {
				case -1: udp_filter_stat.omitted ++; break;
				case  0: udp_filter_stat.accepted++; break;
				case  1: udp_filter_stat.dropped ++; break;
			}
#endif
			if (drop_res == 1) {
#if 0
			if (0) {
				printk("dlink drops it:\n");
				skb_dump(rx_skb);
			}
#endif
/*
#if defined(CONFIG_DRU_UDP_FILTER_STAT)
				if ( (udp_filter_stat.dropped % DMP_DIV) == 0 ) {
					printk("dlink: %llu kpackets dropped\n", udp_filter_stat.dropped >> 10);
				}
#endif
*/
				dru_dropped_frames++;
				ei_local->stat.rx_dropped++;//TODO PSEUDO_SUPPORT ?
				continue;//????? как реально дропнуть пакет??? UPD: вроде так
			}
		}
#endif /* CONFIG_DRU_UDP_FILTER */

#if defined(CONFIG_DRU_IDLE_FILTER)
			if ( dru_sysctl_data.idle_filter.enable ) {
				int drop_res = dlink_checks_cpuidle_to_drop();
#if defined(CONFIG_DRU_IDLE_FILTER_STAT)
				switch (drop_res) {
					case  0: idle_filter_stat.accepted++; break;
					case  1: idle_filter_stat.dropped ++; break;
				}
#endif
				if (drop_res == 1) {
					dru_dropped_frames++;
...
#if defined(CONFIG_DRU_LOAD_FILTER)
			if ( dru_sysctl_data.load_filter.enable ) {
				int drop_res = dlink_checks_cpuload_to_drop();
#if defined(CONFIG_DRU_LOAD_FILTER_STAT)
				switch (drop_res) {
					case  0: load_filter_stat.accepted++; break;
					case  1: load_filter_stat.dropped ++; break;
				}
#endif
				if (drop_res == 1) {
					dru_dropped_frames++;
...
#if 0
было в 2.6.36, выпилено в 3.14.10
...
*/
	...
	dma_unmap_single(dev->dev.parent, rx_ring[rx_dma_owner_idx[rx_ring_no]].rxd_info1.PDP0, length, DMA_FROM_DEVICE);
		...
		// 0xFC78 - маска для определения пакетов со special tag, единицы в ней соответствуют обязательным нулям в special tag
		if (!(ntohs(veth->h_vlan_proto) & 0xFC78))
		{

			port_num = ntohs(veth->h_vlan_proto) & 0xF;
			// восстановление поля протокола по VPM
			uint16_t vpm = ((ntohs(veth->h_vlan_proto) >> 8) & 0x3);
			uint32_t stag_vpid;

			switch (vpm) { // 0x02 пока не рассматривается, так как соответствует Double VLAN
				// untagged, записываем как 8100
				case 0x00:
				...
				case 0x03: // STAG_VPID
					// 31-16 биты в регистрах 2x10 - STAG_VPID
					mii_mgr_read(31, 0x2010 + (port_num << 8), &stag_vpid);
					stag_vpid = stag_vpid >> 16;
					rx_skb->protocol = veth->h_vlan_proto = htons(stag_vpid);
					break;
			}
		}

#elif CONFIG_RALINK_MT7620
...
/* START D-Link Patch #20672 [Ivan Barinov (August 2017)] - Kernel SLA Counters */
		...
		// Если пакеты приходят с вана
		int skb_tag = ((((unsigned char*)(rx_skb->data))[0] << 8) | ((unsigned char*)(rx_skb->data))[1]) & 0xFFF;
		if (skb_tag == wan_vid) {
			// Счётчик RX Broadcast
			struct ethhdr *eth = eth_hdr(rx_skb);
			if ((eth->h_dest[0] == 0xFF) && (eth->h_dest[1] == 0xFF) && (eth->h_dest[2] == 0xFF) &&
				(eth->h_dest[3] == 0xFF) && (eth->h_dest[4] == 0xFF) && (eth->h_dest[5] == 0xFF))
				wan_bcast_rx++;

			// Счётчик фрагментированных пакетов
			iph = ip_hdr(rx_skb);
			if (iph != NULL) {
				if (!(iph->frag_off & 0x4))
					wan_fragments++;
			...
			// Счётчик pause frames
			if (*(uint16_t *)(&(rx_skb->data[ETH_ALEN*2 - 10])) == htons(ETH_P_PAUSE)) {
				wan_pause_rx++;
			}
		} else if (ra_is_lan((uint16_t)skb_tag)) { // Если пакеты приходят с лана
			// Счётчик фрагментированных пакетов
			iph = ip_hdr(rx_skb);
			if (iph != NULL) {
				if (!(iph->frag_off & 0x4))
		...
		// проверяем тип ethernet
		struct ethhdr *lldp_eth_hdr = (struct ethhdr *)(rx_skb->mac_header);

		// если тип 88cc, то вызываем парсер
		if (*(uint16_t *)(&(lldp_eth_hdr->h_proto)) == htons(0x88cc))
		{
#ifndef CONFIG_RAETH_INCOMING_PORT_NUM_DETECTION
			ra_lldp_parse((rx_skb->mac_header + sizeof(struct ethhdr)));
#else
#ifdef CONFIG_WAN_AT_P4
			ra_lldp_parse((rx_skb->mac_header + sizeof(struct ethhdr)), 4);
#else
			ra_lldp_parse((rx_skb->mac_header + sizeof(struct ethhdr)), 0);
#endif // CONFIG_WAN_AT_P4
#endif // CONFIG_RAETH_INCOMING_PORT_NUM_DETECTION
		}

		// если тип 8100 (802.1q), то проверяем тип тегированного пакета
		else if (*(uint16_t *)(&(lldp_eth_hdr->h_proto)) == htons(0x8100))
		{
			struct vlan_hdr *lldp_vlan_hdr = (struct vlan_hdr *)(rx_skb->mac_header + sizeof(struct ethhdr));

			// если тип тегированного пакета 88cc, то вызываем парсер
			if (lldp_vlan_hdr->h_vlan_encapsulated_proto == htons(0x88cc)) {
#ifdef CONFIG_RAETH_INCOMING_PORT_NUM_DETECTION
				if (port_num >= 0) // если порт определён корректно
					ra_lldp_parse(rx_skb->mac_header + sizeof(struct ethhdr) + sizeof(struct vlan_hdr), port_num);
#else
			ra_lldp_parse((rx_skb->mac_header + sizeof(struct ethhdr) + sizeof(struct vlan_hdr)));
#endif // CONFIG_RAETH_INCOMING_PORT_NUM_DETECTION
			}
		}
#endif /* CONFIG_DRU_SLA_COUNTERS */
...
void FASTPATHNET ei_receive_workq(struct work_struct *work)
#else
void FASTPATHNET ei_receive(unsigned long unused)  // device structure
#endif // WORKQUEUE_BH //
{
...
#if defined(CONFIG_DRU_FILTERING)
	dru_total_frames = dru_dropped_frames = 0;//farisey
#endif
...
#ifdef CONFIG_PSEUDO_SUPPORT
static void eth3_link_change(struct net_device *basedev, bool link)
{
	END_DEVICE *ei_local = netdev_priv(basedev);
	struct net_device *eth3 = ei_local->PseudoDev;

	if (link) {
		netif_carrier_on(eth3);
		//netif_wake_queue(eth3);
	} else {
		netif_carrier_off(eth3);
		//netif_tx_disable(eth3);
	}
}
#else
static inline void eth3_link_change(struct net_device *basedev, bool link)
{
}
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,21)
static irqreturn_t FASTPATHNET ei_rx_interrupt(int irq, void *dev_id)
#else
static irqreturn_t FASTPATHNET ei_rx_interrupt(int irq, void *dev_id, struct pt_regs * regs)
#endif
{
...
#endif
//end dlink
#if defined (CONFIG_WAN_AT_P0) || defined (CONFIG_WAN_AT_P4)
    struct net_device *dev = (struct net_device *) dev_id;
    END_DEVICE *ei_local = netdev_priv(dev);
#endif
...
#endif
#ifdef CONFIG_NETLINK_MONITOR
		nlmon_msg.status = NLMON_PORT_ENABLE;
#endif
    } else {
#ifdef CONFIG_NETLINK_MONITOR
		nlmon_msg.status = NLMON_PORT_DISABLE;
#endif
	printk("ESW: Link Status Changed - Port%d Link Down\n", port_no);
    }
#ifdef CONFIG_NETLINK_MONITOR
	nlmon_msg.phy = port_no;
	nlmon_msg.sw = 0;
#ifdef CONFIG_NETLINK_MONITOR_NEW
	nlmon_msg.phy_type = 0;
...
static irqreturn_t esw_interrupt(int irq, void *dev_id)
{
	...
	if (reg_int_val & PORT_ST_CHG) {
		...
		//dlink
#ifdef CONFIG_NETLINK_MONITOR
#define LINK_BIT_IN 25
#define LINK_BIT_OUT 30
		for(i = LINK_BIT_IN; i <= LINK_BIT_OUT; i++) {
			struct nlmon_msg_data nlmon_msg;
			u32 mask = (1<<i);

			if((stat & mask) != (stat_curr & mask)) {
				nlmon_msg.phy = i-LINK_BIT_IN;
				nlmon_msg.sw = 0;
...
/* START D-Link Patch #20672 [Ivan Barinov (August 2017)] - Kernel SLA Counters */
	...
	// если пакет отправляется через wan-интерфейс
	int skb_tag = ((((unsigned char*)(skb->data))[14] << 8) | ((unsigned char*)(skb->data))[15]) & 0xFFF;
	if (skb_tag == wan_vid) {
		// Счётчик TX Broadcast
		if ((((unsigned char*)(skb->data))[0] == 0xFF) && (((unsigned char*)(skb->data))[1] == 0xFF) && (((unsigned char*)(skb->data))[2] == 0xFF) && 
			(((unsigned char*)(skb->data))[3] == 0xFF) && (((unsigned char*)(skb->data))[4] == 0xFF) && (((unsigned char*)(skb->data))[5] == 0xFF)) 
			wan_bcast_tx++;

		// Счётчик TX Multicast
		else if (((unsigned char*)(skb->data))[0] & 1)
			wan_mcast_tx++;

		// если пакет IP
		iph = ip_hdr(skb);
		if (iph != NULL) {
			// Счётчик пакетов > 64 байт
			if (iph->tot_len > 64)
				wan_packets64b++;
	...
	} else if (ra_is_lan((uint16_t)skb_tag)) { // если пакет отправляется через lan-интерфейс
		iph = ip_hdr(skb);
		if (iph != NULL) {
			// Счётчик фрагментированных пакетов на LAN
			if (!(iph->frag_off & 0x4))
				lan_fragments++;
		}
	}
		...
		case RAETH_QDMA_QUEUE_MAPPING:
			...
			spin_lock_irq(&ei_local->page_lock);
				if((reg.off&0x100) == 0x100){
					lan_wan_separate = 1;
					reg.off &= 0xff;//farisey: may be 63 instead of 255 ??
				}else{
					lan_wan_separate = 0;
				}
			__M2Q_table[reg.off % ARRAY_SIZE(__M2Q_table)] = reg.val;
			spin_unlock_irq(&ei_local->page_lock);
	  	break;
#if defined(CONFIG_HW_SFQ)
...
// farisey: /sys/devices/virtual/net/eth[23]/carrier
#ifdef CONFIG_IPV6
static void invalidate_dad(struct net_device *dev)
{
	...
	list_for_each_entry(ifa, &idev->addr_list, if_list) {
		spin_lock(&ifa->state_lock);
		if (ifa->state == INET6_IFADDR_STATE_POSTDAD) {
			ifa->flags |= IFA_F_TENTATIVE;
			ifa->state = INET6_IFADDR_STATE_DAD;
		}
		spin_unlock(&ifa->state_lock);
	}
...
static void __ra_change_carrier(struct net_device *dev, bool new_carrier)
{
	if (new_carrier)
...
static int ra_change_carrier(struct net_device *dev, bool new_carrier)
{
#if IS_ENABLED(CONFIG_VLAN_8021Q)
	struct net_device *d;
#endif

	//rtnl_is_locked() == 1 here, but
	ASSERT_RTNL();

	/* addrconf.c uses rcu under rtnl lock
	 * I'm not sure whether it is correct,
	 * mandatory or redundant (likely), so
	 * follow them.
	 */
	rcu_read_lock();

	if (new_carrier)
		__ra_change_carrier(dev, new_carrier); //UP base first

#if IS_ENABLED(CONFIG_VLAN_8021Q)
	//all VLAN's
	for_each_netdev_rcu(dev_net(dev), d) {
		if (vlan_dev_real_dev(d) == dev)
			__ra_change_carrier(d, new_carrier);
	}
#endif

	if (!new_carrier)
		__ra_change_carrier(dev, new_carrier); //DOWN base last
...
//	.ndo_tx_timeout		= ei_tx_timeout,
	.ndo_change_carrier	= ra_change_carrier,
};
#endif
...
/*
 * #33539, farisey: get MAC from the Factory partition.
 * retval:
 *     < 0 - MTD read error
 *       0 - MAC is invalid
 *       6 - OK
 */
static int ra_get_mac(loff_t offset, uint8_t ether_addr_octet[6])
{
	int res;
	uint8_t x, y;
#ifdef RA_MTD_RW_BY_NUM
	res = ra_mtd_read(2, offset, 6, ether_addr_octet);
#else
	res = ra_mtd_read_nm("Factory", offset, 6, ether_addr_octet);
#endif
	if (res < 0)
		return res;

	/* check if zeroed or 0xff'ed */
	x = 0x00;
	y = 0xff;
	for (res = 0; res < 6; res++) {
		x |= ether_addr_octet[res];
		y &= ether_addr_octet[res];
	}
	if ( (x == 0x00) && (y == 0xff) )
		return 0;

	/* check if multicast */
	if (ether_addr_octet[0] & 0x1)
		return 0;

	return 6;
}

/* may be it presents in a kernel already */
static void adjust_mac(uint8_t ether_addr_octet[6], loff_t adj)
{
	uint32_t x = ntohl(*(uint32_t*)(&ether_addr_octet[2]));
	x += adj;
	x &= 0x00ffffff; /* reset less vendor's octet */
	/* and get the original value back */
	x |= ((uint32_t)ether_addr_octet[2]) << 24;
	/* update the adjusted MAC */
	*(uint32_t*)(&ether_addr_octet[2]) = ntohl(x);
}
...
int __init rather_probe(struct net_device *dev)
{
	...
	i = ra_get_mac(GMAC0_OFFSET, addr.sa_data);

	while (i <= 0) {
		//try to use WiFi address as a base
		i = ra_get_mac(4, addr.sa_data);
		if (i <= 0)
			break; //to generating
		//DRU MAC scheme
		adjust_mac(addr.sa_data, -1); //eth2
		i = 6;
		break;
	}
...
void RAETH_Init_PSEUDO(pEND_DEVICE pAd, struct net_device *net_dev)
{
	...
	i = ra_get_mac(GMAC2_OFFSET, addr.sa_data);
	...
	while (i <= 0) {
		...
		if (i <= 0)
			break; //to generating
		//DRU MAC scheme
		adjust_mac(addr.sa_data, -2); //eth3
		i = 6;
		break;
	}
...
#define RT2860_NVRAM	0
	...
	if (ei_local == NULL)
	{
		...
		ei_local->netrx_skbuf[0][i] = netdev_alloc_skb(dev, MAX_RX_LENGTH + NET_IP_ALIGN);
			...
			//резерв NET_SKB_PAD перенесён сверху
			//патчем patch_sdk_5.0.1.6_20160510_MT7621_hwnat_skb_reserve_patch
			//UPDATE: этот патч есть в sdk 5.0.2.0, но оставлю все
			// эти свои комментарии
#ifdef CONFIG_ETH_SLAB_ALLOC_SKB
...
#if CONFIG_MAC_TO_GIGAPHY_MODE_ADDR == 0x1
#if defined(DAP_1620A1_MT7620A) || defined(DAP_1635A1_MT7620A)
#warning "MII will not be initialized!"
/* dkavanin
 * На профилях DAP_1620A1 и DAP_1635A1, что есть один и тот же девайс, при инициализации MII вырубается порт.
 * Если же инициализацию не делать, то порт вполне себе работает.
 * Выяснить в чём причина у меня до конца не получилось, но думается, что связано это с тем, что на данном устройстве физический адрес у пятого порта 0x1,
 * который тут и настраивается. Тогда как на других утсройствах физический адрес отличен от 0x1.
 * Так как девайс с портом 5 на 0x1 всего один, и именно на нем отваливается порт, то добавил дефайны именно по профилю.
 * Если когда-нибудь порт сломается на другом девавйсе (MAC_TO_GIGAPHY_MODE_ADDR != 0x1) или, наоборот (MAC_TO_GIGAPHY_MODE_ADDR == 0x1) будет работать,
 * то стоит пересмотреть свои взгляды и таки разобраться в чем проблема.
 */
#else
#error "Developer, check ports operation before you start."
#endif
#endif
// #if defined (CONFIG_MT7620_ASIC) && !defined (CONFIG_P5_RGMII_TO_MT7530_MODE)
#if defined (CONFIG_MT7620_ASIC) && !defined (CONFIG_P5_RGMII_TO_MT7530_MODE) && !defined(DAP_1620A1_MT7620A) && !defined(DAP_1635A1_MT7620A)
...
//dkavanin #21188
	 ...
	 */
        ...
        *(unsigned long *)(RALINK_ETH_SW_BASE+0x00CC) = 0x0002500c;
        ...
        /* dlink, dkavanin, #11167 */
        // zero all VLAN members
        /*
         * farisey: merge
         * этот довесок прилетел из 2.6.36, там эта функция
         * находится под пачкой ifdef'ов через ИЛИ:
         * 3052, 3352, 5350 (патч был на 300NRUB7) и 7628
         * Китайцы всё выпилили, но для MT7628 (DWR-711)
         * патч обещает быть нужным
         */
        *(unsigned long *)(RALINK_ETH_SW_BASE+0x0070) = 0x00000000;
        *(unsigned long *)(RALINK_ETH_SW_BASE+0x0074) = 0x00000000;
        *(unsigned long *)(RALINK_ETH_SW_BASE+0x0078) = 0x00000000;
        *(unsigned long *)(RALINK_ETH_SW_BASE+0x007c) = 0x00000000;
...
#ifdef CONFIG_DRU_CANCEL_SW_INIT
	/*VLAN member*/
	printk("empty default vlans\n");
	IsSwitchVlanTableBusy();
	mii_mgr_write(31, 0x94, 0x40400001);//VAWD1
	mii_mgr_write(31, 0x90, 0x80001001);//VTCR, VID=1
	IsSwitchVlanTableBusy();

	mii_mgr_write(31, 0x94, 0x40200001);//VAWD1
...
void setup_internal_gsw(void)
{
	...
	*(volatile u_long *)(ETH_GPIO_BASE+0xed0) = 0;     //set GE2 TUNE
    ...
    defined(CONFIG_NETLINK_MONITOR_WAN_LED_DAP_300P)
//#18026, #20550
static void *wan_led_handle = NULL;
#endif
...
//#if !defined (CONFIG_MIPS)
//farisey: is needed on MIPS due to spinlock.
	END_DEVICE*     ei_local = netdev_priv(dev);
//#endif
...
//farisey:
	spin_lock_init(&ei_local->page_lock);
	spin_lock_init(&ei_local->xmit_lock);
	...
	//#18026
	//WAN на P0 свича
	wan_led_handle = dru_soft_led_register_netdev("wan", dev, 0);
#elif defined(CONFIG_NETLINK_MONITOR_WAN_LED_DIR_853) || defined(CONFIG_NETLINK_MONITOR_WAN_LED_DAP_300P)
	//#20550
	//WAN на P4 свича
	wan_led_handle = dru_soft_led_register_netdev("wan", dev, 4);
#endif
	return ret;
}
...
void ra2882eth_cleanup_module(void)
{
    ...
    defined(CONFIG_NETLINK_MONITOR_WAN_LED_DAP_300P)
	//#18026, #20550
	dru_soft_led_unregister_netdev(wan_led_handle);
#endif
