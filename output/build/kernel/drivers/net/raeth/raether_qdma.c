...
int rt2880_eth_send(END_DEVICE *ei_local, struct net_device* dev, struct sk_buff *skb, int gmac_no)
{
	...
	cpu_ptr->txd_info3.QID = M2Q_table(skb);
	...
	if(dbg==1){
		printk("M2Q_table[%u]=%u\n", skb->mark, M2Q_table(skb));
...
int FASTPATHNET ei_start_xmit(struct sk_buff* skb, struct net_device *dev, int gmac_no)
{
	END_DEVICE *ei_local = netdev_priv(dev);
	unsigned long flags;
