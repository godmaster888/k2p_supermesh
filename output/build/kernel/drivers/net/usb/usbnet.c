...
void usbnet_update_max_qlen(struct usbnet *dev)
{
	...
	case USB_SPEED_SUPER:
		dev->rx_qlen = RX_QLEN(dev);
		dev->tx_qlen = TX_QLEN(dev);
		break;
	default:
...
#ifdef CONFIG_DRU_USB_ARP_CASTRATE
	if ((eth_hdr(skb)->h_proto == htons(ETH_P_ARP)) && (skb->len >=60)) {
		skb_trim(skb, 42); // eth(14) + arp(28)
	}
#endif
