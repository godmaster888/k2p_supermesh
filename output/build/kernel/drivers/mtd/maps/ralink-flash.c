...
//dlink
				...
				name:           "Linux2",
				size:           IMAGE1_SIZE - SAFEMODE_SIZE,
				offset:         IMAGE1_SIZE + SAFEMODE_SIZE,
#endif
//end dlink
        }
};
...
static inline loff_t search_root_fs(loff_t start, loff_t size) {
	...
	for (; off < size; off += 0x10000) {
		...
		ptr = (char *)CKSEG1ADDR(CONFIG_MTD_PHYSMAP_START + off);
		memset(&squashfsb, 0xe5, sizeof(squashfsb));
		memcpy(&squashfsb, ptr, sizeof(squashfsb));
		...
		if ((squashfsb.s_magic == 0x71736873) || (squashfsb.s_magic == 0x73717368)
			...
			printk(KERN_NOTICE
					"squashfs filesystem found at block 0x%x\n", (int) off);
			break;
		}
	}
	return off;
...
#ifdef CONFIG_DUAL_IMAGE
	loff_t off2 = search_root_fs(off + rt2880_partitions[5].size + 0xD0000 + SAFEMODE_SIZE, 2*IMAGE1_SIZE);

	rt2880_partitions[8].offset = SAFEMODE_ADDR + SAFEMODE_SIZE;          // Kernel2
	rt2880_partitions[8].size = off2 - (SAFEMODE_ADDR + SAFEMODE_SIZE);
	rt2880_partitions[9].offset = off2;                                 // RootFS2
	rt2880_partitions[9].size = 2*IMAGE1_SIZE - off2;
#endif
// end dlink
