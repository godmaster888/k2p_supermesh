	...
	// Put your own partition definitions here 
        {
                ...
                name:           "RootFS",
                size:           MTDPART_SIZ_FULL - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + MTD_KERN_PART_SIZE),
                offset:         MTDPART_OFS_APPEND,
        }, {
                name:           "Linux", // mtdblock5 
...
#if defined (CONFIG_JFFS2_PARTITION_512K)
#if defined (CONFIG_RT2880_FLASH_2M)
#define MTD_JFFS2_PART_SIZE	0x00000
...
static struct mtd_partition rt2880_partitions[] = {
	{
			...
			name:           "Linux",
			size:           MTD_KERN_PART_SIZE - MTD_JFFS2_PART_SIZE,
			offset:         MTDPART_OFS_APPEND,
	}, {
			name:           "RootFS",
			size:           MTD_ROOTFS_PART_SIZE - MTD_JFFS2_PART_SIZE,
			offset:         MTDPART_OFS_APPEND,
	}, {
			name:           "Kernel",
			size:           MTD_KERN_PART_SIZE - MTD_JFFS2_PART_SIZE,
			offset:         MTDPART_OFS_APPEND,
#if MTD_JFFS2_PART_SIZE > 0x00000
	}, {
			name:           "JFFS2",
			size:           MTD_JFFS2_PART_SIZE,
			offset:         MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + MTD_KERN_PART_SIZE,
#endif
	}
};
...
static int __devinit raspi_prob(void)
{
	...
	off = MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + 0x80000;
...
//#if defined (CONFIG_RT2880_ROOTFS_IN_FLASH) && defined (CONFIG_ROOTFS_IN_FLASH_NO_PADDING)
	...
	for (off; off < flash->mtd.size; off += flash->mtd.erasesize) {
		memset(&squashfsb, 0xe5, sizeof(squashfsb));
		/*
		 * Read block 0 to test for romfs and cramfs superblock
		 */
		ramtd_read(NULL, off, sizeof(squashfsb), (size_t *)&i, (u_char *)(&squashfsb));
/*
		printk(KERN_NOTICE
					"magic %x, off = %x\n", squashfsb.s_magic, off);
*/
	...
	rt2880_partitions[4].size = flash->mtd.size - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + MTD_JFFS2_PART_SIZE);
	rt2880_partitions[6].size = flash->mtd.size - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + MTD_JFFS2_PART_SIZE);
	rt2880_partitions[6].offset = MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE;
	rt2880_partitions[5].size = flash->mtd.size - off - MTD_JFFS2_PART_SIZE;
#if MTD_JFFS2_PART_SIZE > 0x00000
	rt2880_partitions[7].offset = flash->mtd.size - MTD_JFFS2_PART_SIZE;
	rt2880_partitions[7].size = MTD_JFFS2_PART_SIZE;
#endif
