...
#	if defined(CONFIG_DRU_RALINK_DYN_ROOTFS)
                ...
                size:           IMAGE1_SIZE - MTD_BOOT_PART_SIZE - MTD_CONFIG_PART_SIZE - MTD_FACTORY_PART_SIZE,
...
#	if defined(CONFIG_DRU_RALINK_DYN_ROOTFS)
		...
		name:		"SafeMode", // раздел для хранения параметров безопасного режима
		size:		SAFEMODE_SIZE,
...
#define OPCODE_DIES		0xC2	/* Die Select */

#define DIESZ(flash)		(flash->chip->sector_size * flash->chip->n_sectors / 2)
/* Check if the flash is W25Q512JV */
#define IS_TWODIE(flash)	((flash->chip->id == 0xef) && (flash->chip->jedec_id == 0x71190000))
/* Check if need to switch to second die(each die is 32MB) */
#define IS_SECDIE(flash, addr, len) 		((addr+len) > DIESZ(flash))

#define MASK_ADDR(addr)		(addr & 0x1ffffff)
...
static int raspi_die_select(u8 die)
{
	if (die > 1)
	{
		printk("Die select fail, die=%d\n", die);
		return -1;
	}
	raspi_wait_ready(1);
	raspi_write_rg(OPCODE_DIES, &die);
	raspi_wait_ready(1);
	return 0;
}
...
#endif
	if (IS_TWODIE(flash) && IS_SECDIE(flash, addr, len))
	{
		if (IS_SECDIE(flash, addr+1, 0))
		{
			addr -= DIESZ(flash);
			raspi_die_select(1);
		}
	...
	size_t *retlen, u_char *buf)
{
	...
	if (IS_TWODIE(flash) && IS_SECDIE(flash, from, len))
	{
		if (IS_SECDIE(flash, from+1, 0))
		{
			from -= DIESZ(flash);
			raspi_die_select(1);
		}
		...
		{
			printk("Error: unhandle cross boundary access\n");
		}
	}
...
#endif
	if (IS_TWODIE(flash) && IS_SECDIE(flash, to, len))
	{
		if (IS_SECDIE(flash, to+1, 0))
		{
			to -= DIESZ(flash);
			raspi_die_select(1);
		}
...
#if defined(CONFIG_DRU_RALINK_DYN_ROOTFS)
static loff_t search_root_fs(loff_t start, loff_t size)
{
	// Look at every 64 KB boundary
	for (; start < size; start += 0x10000) {

		struct squashfs_super_block squashfsb;

		// create empty mtd structure to use in ramtd_write
		struct mtd_info mtd;
		size_t retlen;

		memset(&mtd, 0x00, sizeof(mtd));
		retlen = 0;

		ramtd_read(&mtd, start, sizeof(struct squashfs_super_block), &retlen, (u_char *)&squashfsb);

		printk(KERN_NOTICE "off at block 0x%x, squashfsb.s_magic 0x%x\n", (unsigned)start, squashfsb.s_magic);

		switch (squashfsb.s_magic) {
			case SQUASHFS_MAGIC:			//0x73717368
			/* case __constant_swab32(SQUASHFS_MAGIC):	//0x68737173 */
			case 0x71736873:
			case 0x73687371:
...
static inline void dru_patch_partitions(void)
{
	loff_t off = search_root_fs(MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + 0xD0000, IMAGE1_SIZE);
	if (!off) {
		printk(KERN_EMERG "cant find rootfs!\n");
	} else {
		rt2880_partitions[5].offset = off;
		rt2880_partitions[4].size = IMAGE1_SIZE - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE) - (IMAGE1_SIZE - off);
		rt2880_partitions[5].size = IMAGE1_SIZE - off;
	}

#if 1
#ifdef CONFIG_DUAL_IMAGE
	off = search_root_fs(MTD_KERN2_PART_OFFSET  + 0xD0000, 2*IMAGE1_SIZE);
	if (!off) {
		printk(KERN_EMERG "cant find rootfs2!\n");
	} else {
		rt2880_partitions[9].offset = MTD_CONFIG2_PART_OFFSET + MTD_CONFIG_PART_SIZE; // Kernel2
		rt2880_partitions[9].size = off - rt2880_partitions[9].offset;
		rt2880_partitions[10].offset = off; // RootFS2
		rt2880_partitions[10].size = 2 * IMAGE1_SIZE - off;
	}

#endif
#endif
}
#endif	/* CONFIG_DRU_RALINK_DYN_ROOTFS */
