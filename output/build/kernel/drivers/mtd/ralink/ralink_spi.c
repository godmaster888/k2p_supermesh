...
#ifdef CONFIG_DRU_DWR_921C1_BOOTSTRAP
        }, {
                name:           "oldroot",
                size:           0xd90000,
                offset:         0x180000,
...
static struct chip_info chips_data [] = {
	...
	{ "GD25Q256DYIG",       0xC8, 0x4019c840, 64 * 1024, 512, 1 },
...
static inline loff_t search_root_fs(loff_t start, loff_t size) {
	...
	for (; off < size; off += 0x10000) {

		memset(&mtd, 0x00, sizeof(mtd));
		retlen = 0;

		ramtd_read(&mtd, off, sizeof(struct squashfs_super_block), &retlen, (u_char *)&squashfsb);

		printk(KERN_NOTICE "off at block 0x%x, squashfsb.s_magic 0x%x\n", (unsigned)off, squashfsb.s_magic);
