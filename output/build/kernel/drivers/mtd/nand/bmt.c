...
bool nand_erase_bmt(u32 offset)
{
    ...
    // nand_get_device is necessary for correct work while bmt reconstruction
    if (nand_chip_bmt->state == FL_READY) {
        nand_get_device(mtd_bmt, FL_ERASING);
        locked = true;
    }
    status = mtk_nand_erase_hw(mtd_bmt, offset / PAGE_SIZE_BMT); // as nand_chip structure doesn't have a erase function defined
    if (locked)
        nand_release_device(mtd_bmt);
...
bool update_bmt(u32 offset, update_reason_t reason, u8 * dat, u8 * oob)
{
    ...
    switch (reason) {
        case UPDATE_ERASE_FAIL:
            MSG(INIT, "Try to update bmt because of erase fail\n");
            break;
        case UPDATE_WRITE_FAIL:
            MSG(INIT, "Try to update bmt because of write fail\n");
            break;
        case UPDATE_UNMAPPED_BLOCK:
            MSG(INIT, "Try to update bmt because of unmapped block\n");
            break;
        default:
            break;
	}
    ...
    if (bad_index >= system_block_count)    // mapped block become bad, find original bad block
    {
        for (i = 0; i < bmt_block_count; i++)
        {
            if (bmt.table[i].mapped_index == bad_index)
            {
                ...
                MSG(INIT, "Change %d BMT entry: bad mapped index %x to new %x\n", i, bmt.table[i].mapped_index, map_index);
