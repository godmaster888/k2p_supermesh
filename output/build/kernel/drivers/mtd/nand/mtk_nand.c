...
// dlink
#if defined(CONFIG_DRU_RALINK_DYN_ROOTFS)
...
/* BAD_BLOCK_RESERVE is size of additional space that is used in Image
 * partition for correct work of bad blocks skip */

#if defined(CONFIG_PART_FOR_BB_SKIP)
#define BAD_BLOCK_RESERVE_COUNT		10
#define BAD_BLOCK_RESERVE			(CFG_BLOCKSIZE * BAD_BLOCK_RESERVE_COUNT)
...
#define SAFEMODE_NUM	7
#define CONF2_NUM		8
#define KERN2_NUM		9
#define ROOTFS2_NUM		10
#define LINUX2_NUM		11
...
#if defined(CONFIG_PART_FOR_BB_SKIP)
#if defined(CONFIG_DUAL_IMAGE)
#define FILE_SYSTEM_START_ADDRESS	((IMAGE1_SIZE * 2) + BAD_BLOCK_RESERVE)
#else //not CONFIG_DUAL_IMAGE
#define FILE_SYSTEM_START_ADDRESS	(IMAGE1_SIZE + BAD_BLOCK_RESERVE)
#endif //CONFIG_DUAL_IMAGE
#else // not CONFIG_PART_FOR_BB_SKIP
#if defined(CONFIG_DUAL_IMAGE)
#define FILE_SYSTEM_START_ADDRESS	(IMAGE1_SIZE * 2)
#else //not CONFIG_DUAL_IMAGE
...
#define NAND_MTD_ROOTFS_PARTITION_NO            ROOTFS_NUM
#define NAND_MTD_LINUX_PARTITION_NO             LINUX_NUM
#ifdef CONFIG_DUAL_IMAGE
#define NAND_MTD_ROOTFS2_PARTITION_NO           ROOTFS2_NUM
#define NAND_MTD_LINUX2_PARTITION_NO            LINUX2_NUM
#endif // CONFIG_DUAL_IMAGE
#endif
...
static int get_start_end_block(struct mtd_info *mtd, int block, int *start_blk, int *end_blk)
{
	struct nand_chip *chip = mtd->priv;
#if defined(CONFIG_PART_FOR_BB_SKIP)
	/* if Image partition is used, it includes every other
	 * partitions, so borders for every block are borders of Image,
	 * not of every smaller partition */

	// check Image partition
	*start_blk = 0;
#if defined(CONFIG_DUAL_IMAGE)
	*end_blk = ((IMAGE1_SIZE_NEW * 2 + BAD_BLOCK_RESERVE) >> chip->phys_erase_shift) - 1;
#else
	*end_blk = ((IMAGE1_SIZE_NEW + BAD_BLOCK_RESERVE) >> chip->phys_erase_shift) - 1;
#endif
	...
	return -1;
#else // not defined CONFIG_PART_FOR_BB_SKIP
	int i;
...
#ifdef CONFIG_RT2880_ROOTFS_IN_FLASH
		// skip Linux and (if exists) Linux2 partition entries
		else if ((i == NAND_MTD_LINUX_PARTITION_NO)
#ifdef CONFIG_DUAL_IMAGE
		      || (i == NAND_MTD_LINUX2_PARTITION_NO)
#endif //CONFIG_DUAL_IMAGE
              )
		{
			continue;
...
#ifdef CONFIG_DUAL_IMAGE
			|| (i == (NAND_MTD_ROOTFS2_PARTITION_NO-1))
#endif //CONFIG_DUAL_IMAGE
			)
			{
...
#ifdef CONFIG_DUAL_IMAGE
			|| (i == NAND_MTD_ROOTFS2_PARTITION_NO)
#endif
			)
			{
...
static int is_skip_bad_block(struct mtd_info *mtd, int page)
{
#ifndef CONFIG_MTK_NAND_REMAP_SUPPORT
	struct nand_chip *chip = mtd->priv;
	...
	return 1;
#else
	/* remap is used instead of skip, so this function
...
// dlink (insert code)
#if defined(CONFIG_DRU_RALINK_DYN_ROOTFS)
static inline loff_t search_root_fs(struct mtd_info *mtd, struct nand_chip *chip, loff_t start, loff_t size)
{
	loff_t addr = 0;
	struct squashfs_super_block squashfsb;
	const loff_t block_size = (1UL << chip->phys_erase_shift); //размер блока в байтах => 128k
	const unsigned int page_per_block = (1U << (chip->phys_erase_shift - chip->page_shift)); //страниц в блоке, 128k/2k = 64
	loff_t start_block = start >> chip->phys_erase_shift; //блок начала "Linux" и "Kernel"
	loff_t stop_block = (start + size) >> chip->phys_erase_shift; // 0x000000140000-0x000002000000 : "Linux"
	loff_t block = start_block + ((1024 * 1024) >> chip->phys_erase_shift); // скипнем метр от начала "Linux"
	u8 *buf = kmalloc(block_size, GFP_KERNEL);

	if (unlikely(!buf)) {
		//всё хуёво, памяти не дали, выйти и умереть
		return 0;
	}

	for (; block < stop_block; block++) {
		int page = block * page_per_block; //номер начальной страницы блока
#ifndef CONFIG_MTK_NAND_REMAP_SUPPORT
		int ret;
#endif
		addr = ((loff_t)page) << chip->page_shift; //её адрес в байтах
#ifndef CONFIG_MTK_NAND_REMAP_SUPPORT
		...
		mtk_nand_read_page(mtd, chip, buf, page);

		memset(&squashfsb, 0xe5, sizeof(squashfsb));
		memcpy(&squashfsb, buf, sizeof(squashfsb));

		printk(KERN_NOTICE
		       "squashfsb.s_magic 0x%x\n", squashfsb.s_magic); //0x71736873

		/* squashfs is at block zero too */
		if ((squashfsb.s_magic == 0x71736873) || (squashfsb.s_magic == 0x73717368)
		    || (squashfsb.s_magic == 0x73687371)) {
			printk(KERN_NOTICE "squashfs filesystem found at block 0x%x\n", (int) addr);
			kfree(buf);
			return addr;
		}
	}
...
#endif
{
...
#endif	
#else
#if defined (__KERNEL_NAND__) && !defined(CONFIG_SUPPORT_OPENWRT)
	/* modify partition table */
	/* redefine boot, config and factory sizes to large values and all parts that are depending on them */
	/* also use IMAGE1_SIZE_NEW that is calculated from mtd->size in partitions.h instead of IMAGE1_SIZE*/

	g_pasStatic_Partition[BOOT_NUM].size = LARGE_MTD_BOOT_PART_SIZE; 		// bootloader
	g_pasStatic_Partition[CONF_NUM].size = LARGE_MTD_CONFIG_PART_SIZE;		// config
	g_pasStatic_Partition[FACT_NUM].size = LARGE_MTD_FACTORY_PART_SIZE;	// factory

	// rootfs
	g_pasStatic_Partition[ROOTFS_NUM].size = IMAGE1_SIZE_NEW - (g_pasStatic_Partition[BOOT_NUM].size + g_pasStatic_Partition[CONF_NUM].size \
								  + g_pasStatic_Partition[FACT_NUM].size + g_pasStatic_Partition[KERN_NUM].size);

	// linux
	g_pasStatic_Partition[LINUX_NUM].size = IMAGE1_SIZE_NEW - g_pasStatic_Partition[BOOT_NUM].size - g_pasStatic_Partition[CONF_NUM].size \
	                              - g_pasStatic_Partition[FACT_NUM].size;
	g_pasStatic_Partition[LINUX_NUM].offset = g_pasStatic_Partition[BOOT_NUM].size + g_pasStatic_Partition[CONF_NUM].size \
	                                + g_pasStatic_Partition[FACT_NUM].size;

#ifdef CONFIG_DUAL_IMAGE
	// safemode
	g_pasStatic_Partition[SAFEMODE_NUM].size = g_pasStatic_Partition[BOOT_NUM].size + g_pasStatic_Partition[FACT_NUM].size;
	g_pasStatic_Partition[SAFEMODE_NUM].offset = IMAGE1_SIZE_NEW;

	// config 2
	g_pasStatic_Partition[CONF2_NUM].size   = LARGE_MTD_CONFIG_PART_SIZE;
	g_pasStatic_Partition[CONF2_NUM].offset = g_pasStatic_Partition[SAFEMODE_NUM].offset + g_pasStatic_Partition[SAFEMODE_NUM].size;

	// kernel 2
	g_pasStatic_Partition[KERN2_NUM].offset = g_pasStatic_Partition[CONF2_NUM].offset + g_pasStatic_Partition[CONF2_NUM].size;

	// rootfs 2
	g_pasStatic_Partition[ROOTFS2_NUM].size = g_pasStatic_Partition[ROOTFS_NUM].size;
	g_pasStatic_Partition[ROOTFS2_NUM].offset =  g_pasStatic_Partition[KERN2_NUM].offset + g_pasStatic_Partition[KERN2_NUM].size;

	// linux 2
	g_pasStatic_Partition[LINUX2_NUM].size = IMAGE1_SIZE_NEW - (g_pasStatic_Partition[SAFEMODE_NUM].size + g_pasStatic_Partition[CONF2_NUM].size);
	g_pasStatic_Partition[LINUX2_NUM].offset = g_pasStatic_Partition[KERN2_NUM].offset;
#endif

#if defined(CONFIG_PART_FOR_BB_SKIP)
	// image
#ifdef CONFIG_DUAL_IMAGE
	g_pasStatic_Partition[IMAGE_NUM].size = (IMAGE1_SIZE_NEW * 2) + BAD_BLOCK_RESERVE;
#else
	g_pasStatic_Partition[IMAGE_NUM].size = IMAGE1_SIZE_NEW + BAD_BLOCK_RESERVE;
#endif // CONFIG_DUAL_IMAGE
#endif // CONFIG_PART_FOR_BB_SKIP
...
#if defined(CONFIG_DRU_RALINK_DYN_ROOTFS)
	// dlink
	// нужно пофиксить размер разделов, так как IMAGE1_SIZE - это не вся флешка
#if defined(SKIP_BAD_BLOCK)
	shift_on_bbt = 1;
#endif
	off = search_root_fs(mtd, nand_chip, g_pasStatic_Partition[LINUX_NUM].offset, IMAGE1_SIZE_NEW);
#if defined(SKIP_BAD_BLOCK)
	shift_on_bbt = 0;
#endif
	if (!off)
		printk(KERN_EMERG "cant find rootfs!\n");
	else {
		// kernel
		g_pasStatic_Partition[KERN_NUM].size = IMAGE1_SIZE_NEW - (LARGE_MTD_BOOT_PART_SIZE + LARGE_MTD_CONFIG_PART_SIZE + LARGE_MTD_FACTORY_PART_SIZE) - (IMAGE1_SIZE_NEW - off);

		// rootfs
		g_pasStatic_Partition[ROOTFS_NUM].offset = off;
		g_pasStatic_Partition[ROOTFS_NUM].size = IMAGE1_SIZE_NEW - off;
	}
	...
	shift_on_bbt = 1;
#endif
	off = search_root_fs(mtd, nand_chip, g_pasStatic_Partition[KERN2_NUM].offset, IMAGE1_SIZE_NEW);
#if defined(SKIP_BAD_BLOCK)
	shift_on_bbt = 0;
#endif
	if (!off) {
		printk(KERN_EMERG "cant find rootfs2!\n");
	}
    else {
		// kernel 2
		g_pasStatic_Partition[KERN2_NUM].size = off - g_pasStatic_Partition[KERN2_NUM].offset;

		// rootfs 2
		g_pasStatic_Partition[ROOTFS2_NUM].offset = off;
		g_pasStatic_Partition[ROOTFS2_NUM].size = 2 * IMAGE1_SIZE_NEW - off;
	}
#endif
	// dlink end
#else
