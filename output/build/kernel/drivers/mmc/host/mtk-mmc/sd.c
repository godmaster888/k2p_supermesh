...
static int msdc_tune_bread(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	/* MT7628 has ECO = 0, but does not reverse this register on read/write */
...
static void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    if (host->mclk != ios->clock) {
	    ...
	    sdr_set_field(MSDC_IOCON, 1<<8, hw->data_edge);
            //} /* for tuning debug */
        } else { /* default value */
...
#else  
    reg = sdr_read32((volatile u32*)(RALINK_SYSCTL_BASE + 0x60)) & ~(0x3<<0) & ~(0x3<<6) & ~(0x3<<10) & ~(0x1<<15) & ~(0x3<<20) & ~(0x3<<24);
    reg = reg | (0x1<<0) | (0x1<<6) | (0x1<<10) | (0x1<<15) | (0x1<<20) | (0x1<<24);
   // reg = 0x55158448;
