...
static const struct usb_device_id id_table[] = {
	...
	{USB_DEVICE(0x1435, 0xd181)},	/* Unknown on D-Link DWR-921 rev.C1 */
	{USB_DEVICE(0x2020, 0x2033)},	/* Unknown on D-Link DWR-921 rev.C3 */
...
static int qcprobe(struct usb_serial *serial, const struct usb_device_id *id)
{
	...
	//farisey
	if ( (nintf == 5) || (nintf == 7) )
	if ( ((id->idVendor == 0x1435) && (id->idProduct == 0xd181)) ||
	     ((id->idVendor == 0x2020) && (id->idProduct == 0x2033)) ) {
		dev_info(dev, "!gotta LTE module @ D-Link DWR-921 rev.C%d (is_gobi1k = %d)!\n",
		         (id->idProduct == 0xd181) ? 1 : 3, (int)is_gobi1k);
		...
		if (ifnum == 2) {
			...
			dev_warn(dev, "skip GPS port (intf = %d) because of close() w/a\n", ifnum);
			//TODO fix it
			altsetting = -1;
		}
		goto done;
