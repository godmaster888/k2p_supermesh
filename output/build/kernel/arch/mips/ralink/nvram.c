...
static char *nvram_strnchr(const char *s, size_t count, int c)
{
	for (; count--; ++s)
		if (*s == (char)c)
			return (char *)s;
...
static int init_nvram_block(int index)
{
	...
	if(!fb[i].env.data)
		fb[i].env.data = (char *)kzalloc(envDataLen, GFP_KERNEL);
	if (!fb[i].env.data)
		return -ENOMEM;
	...
	for (j = 0; j < MAX_CACHE_ENTRY; j++) {
		if (NULL == (q = nvram_strnchr(p,end-p, '='))) {
		...
		*q = '\0'; //strip '='
		...
		p = q + 1; //value
		if (NULL == (q = nvram_strnchr(p, end-p,'\0'))) {
