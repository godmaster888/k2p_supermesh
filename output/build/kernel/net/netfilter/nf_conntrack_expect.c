...
/*
 * refresh timer for expctations mastered by @ct
 * based on nf_ct_remove_expectations()
 * returns number of refreshed conntracks
 * used in rtsp_conntrack (designed for it)
 */
/*
 * refresh_timer is absent in 3.10.14
 */
...
int nf_ct_refresh_expectations(struct nf_conn *ct)
{
	struct nf_conn_help *help = nfct_help(ct);
	struct nf_conntrack_expect *exp;
	struct hlist_node *next;
	...
	hlist_for_each_entry_safe(exp, next, &help->expectations, lnode) {
		if (exp->master == ct) {
			res += refresh_timer(exp);
		}
	}
	return res;
}
EXPORT_SYMBOL_GPL(nf_ct_refresh_expectations);
//farisey end
