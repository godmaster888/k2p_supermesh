...
#if defined(CONFIG_NETFILTER_XT_MATCH_URL) || defined(CONFIG_NETFILTER_XT_MATCH_URL_MODULE)
#include <net/netfilter/xt_url.h>
unsigned int xt_url_loaded __read_mostly = 0;
EXPORT_SYMBOL_GPL(xt_url_loaded);
#else
/* never used below, but compiler needs it */
#define xt_url_method_offset(x) (-1)
#endif

#if defined(CONFIG_BCM_NAT)
#include <net/ip.h>	/* IP_MF, IP_OFFSET, ...*/
//#include <linux/netfilter_ipv4.h>	/* NF_IP_PRE_ROUTING NO! for userspace only */
#define NF_IP_PRE_ROUTING	NF_INET_PRE_ROUTING
//#define NF_IP_LOCAL_OUT		NF_INET_LOCAL_OUT see below
#endif
...
unsigned int nf_conntrack_sip_bypass_hwnat __read_mostly = 0;
EXPORT_SYMBOL_GPL(nf_conntrack_sip_bypass_hwnat);
#endif
...
unsigned int nf_conntrack_count_sum;
EXPORT_SYMBOL_GPL(nf_conntrack_count_sum);

unsigned int nf_conntrack_period_peak;
EXPORT_SYMBOL_GPL(nf_conntrack_period_peak);
#endif
...
/* Added by Steven Liu */
#if defined (CONFIG_NAT_FCONE) || defined (CONFIG_NAT_RCONE)
static FASTPATHNET struct nf_conntrack_tuple_hash *
...
FASTPATHNET struct nf_conntrack_tuple_hash *
...
static void FASTPATHNET __nf_conntrack_hash_insert(struct nf_conn *ct,
				       unsigned int hash,
		     ...
		     gfp_t gfp, u32 hash)
{
	...
	/* START D-Link Patch #13703 [Ksenia Korabelnikova (July 2016)] / #20672 [Ivan Barinov (August 2017)] */
#ifdef CONFIG_DRU_SLA_COUNTERS
	nf_conntrack_count_sum++;
	if (atomic_read(&net->ct.count) > nf_conntrack_period_peak) {
		nf_conntrack_period_peak = atomic_read(&net->ct.count);
	}
#endif
	/* END D-Link Patch */
	...
	if (nf_conntrack_max &&
		...
		#ifndef CONFIG_DRU_NO_CONNFULL_LOG
...
unsigned int FASTPATHNET
nf_conntrack_in(struct net *net, u_int8_t pf, unsigned int hooknum,
		struct sk_buff *skb)
{
	struct nf_conn *ct, *tmpl = NULL;
#if defined(CONFIG_RA_HW_NAT) || defined(CONFIG_RA_HW_NAT_MODULE) || defined(CONFIG_BCM_NAT)
	struct nf_conn_help *help; //#4027, перенесено вниз внутрь if. UPDATE вернул, т.к. убрал #if 0
	int skip_offload = 0;
#if defined(CONFIG_RA_HW_NAT) || defined(CONFIG_RA_HW_NAT_MODULE)
	extern int (*ra_sw_nat_hook_rx) (struct sk_buff * skb);
#	ifndef FOE_ALG_MARK
#		define FOE_ALG_MARK(skb)	do { if (IS_SPACE_AVAILABLED(skb) && IS_MAGIC_TAG_VALID(skb)) FOE_ALG(skb) = 1; } while (0)
#	endif
#endif
#if defined(CONFIG_BCM_NAT)
	int pure_route = 0;
	...
	if ((nf_conntrack_fastnat || nf_conntrack_fastroute) && (pf == PF_INET))
		...
		if (ip_hdr(skb)->frag_off & htons(IP_MF|IP_OFFSET))
			if(nf_ct_ipv4_gather_frags(skb, hooknum == NF_IP_PRE_ROUTING ? IP_DEFRAG_CONNTRACK_IN : IP_DEFRAG_CONNTRACK_OUT)) {
				ret = NF_STOLEN;
				goto out;
			}
#endif
...
#warning farisey: it was disabled in d82a30a9 while porting from wive ...
#warning farisey: TODO: TEST THIS FOR HWNAT (and porttrigger) in both variants (en/dis abled)
//#if 0
#if  defined(CONFIG_RA_HW_NAT) || defined(CONFIG_RA_HW_NAT_MODULE) || defined(CONFIG_BCM_NAT)
	help = nfct_help(ct);
	if (help && help->helper) {
...
/* wive code */
	...
	/* TRIGGER conntrack */
	if (ct->dru_skip_offloads)
		skip_offload = 1;//goto skip; простой goto не прокатывает
#endif

	/*
	 * skip ALG marked packets from all fastpaths
	 */
	//farisey: нет смысла проверять, если выше выставлено.
	//Если help нужен за пределами, то раскомментить в начале
	//функи и соотв. убрать этот if.
	if (!skip_offload) {
		//farisey: этот код фактически из коммита d82a30a9
...
#endif
	    && FOE_ALG(skb) == 0 && (skip_offload || is_local_svc(protonum))) {
		FOE_ALG_MARK(skb);
	}
#endif /* RA_HW_NAT[_MODULE] */
	 ...
	 */
	if (FASTNAT_SKIP_PROTO(protonum) || FASTNAT_SKIP_TYPE(skb) || (pf != PF_INET))
	    goto skip;
	 ...
	 */
        if (nf_conntrack_fastroute && !skip_offload && skb_is_ready(skb) && pure_route && FASTNAT_ESTABLISHED(ctinfo)) {
    	    int bcmverdict = bcm_do_fastroute(ct, skb, hooknum, set_reply);
	...
	if (
	    ...
	    if (xt_url_loaded && protonum == IPPROTO_TCP && CTINFO2DIR(ctinfo) == IP_CT_DIR_ORIGINAL) {
		struct tcphdr _tcph, *tcph;
		unsigned char _data[8], *data;
...
pass:
		...
		if (hooknum != NF_IP_LOCAL_OUT && FOE_ALG(skb) == 0)
		    FOE_ALG_MARK(skb);
#endif
#ifdef CONFIG_BCM_NAT
	 ...
	 */
	if (!skip_offload) //farisey: #4027, обнаружилось, что скипнутые попадают в bcm nat
...
bool FASTPATHNET nf_ct_invert_tuplepr(struct nf_conntrack_tuple *inverse,
			  const struct nf_conntrack_tuple *orig)
{
	bool ret;
...
//farisey: #5843, #5688, #10561
/*
 * refresh related conntracks to future with @extra_jiffies
 * based on get_next_corpse()
 * returns number of refreshed conntracks
 * TODO optimize search within tuples (?)
 * used in rtsp_conntrack (designed for it)
 */
int
nf_ct_refresh_related(struct nf_conn *master, unsigned long extra_jiffies)
{
	struct nf_conntrack_tuple_hash *h;
	struct nf_conn *ct;
	struct hlist_nulls_node *n;
#ifdef CONFIG_NET_NS
	struct net *net = master->ct_net;
#else
	...
	for (; bucket < net->ct.htable_size; bucket++) {
		hlist_nulls_for_each_entry(h, n, &net->ct.hash[bucket], hnnode) {
			ct = nf_ct_tuplehash_to_ctrack(h);
			if (ct->master != master) continue;
			if (!del_timer(&ct->timeout)) continue;
			ct->timeout.expires = jiffies + extra_jiffies;
#ifdef HNDCTF
			//bcm only
			ct->expire_jiffies = ct->timeout.expires;
#endif
			add_timer(&ct->timeout);
			res++;
		}
...
EXPORT_SYMBOL_GPL(nf_ct_refresh_related);
//end farisey
