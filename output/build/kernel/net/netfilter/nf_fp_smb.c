...
static int __net_init nf_fp_smb_net_init(struct net *net)
{
#ifdef CONFIG_PROC_FS
	if (!proc_create("nf_fp_smb", 0664, net->nf.proc_netfilter, &nf_fp_smb_fops))
		return -ENOMEM;
#endif
	return 0;
}

static void __net_exit nf_fp_smb_net_exit(struct net *net)
{
#ifdef CONFIG_PROC_FS
...
static struct pernet_operations nf_fp_smb_net_ops = {
	.init = nf_fp_smb_net_init,
	.exit = nf_fp_smb_net_exit,
};
...
int __init netfilter_fp_smb_init(void)
{
	...
	ret = register_pernet_subsys(&nf_fp_smb_net_ops);
	if (ret < 0)
		return ret;
