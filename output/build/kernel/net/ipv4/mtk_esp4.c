...
bool ipsec_acc_enable = false;
extern int esp_output(
	struct xfrm_state *x,
	struct sk_buff *skb
);
extern int esp_input(
	struct xfrm_state *x,
	struct sk_buff *skb
);
extern int esp6_output(
	struct xfrm_state *x,
	struct sk_buff *skb
);
...
EXPORT_SYMBOL(ipsec_eip93Adapter_mark_free);
EXPORT_SYMBOL(ipsec_acc_enable);
...
)
{
	...
	{
#if defined (CONFIG_HWCRYPTO_MEMPOOL)
		if ((spi_outbound_tbl[i] == spi) || (spi==0xFFFFFFFF))
		{
	...
	{
#if defined (CONFIG_HWCRYPTO_MEMPOOL)
		if ((spi_inbound_tbl[i] == spi) || (spi==0xFFFFFFFF))
		{
...
#if !defined (MTK_EIP97_DRIVER)
	if ((ipsec_acc_enable==false) || (!(x->type->flags&XFRM_TYPE_HW_ACC)))
...
#if !defined (MTK_EIP97_DRIVER) && (defined (CONFIG_INET6_ESP) || defined (CONFIG_INET6_ESP_MODULE))
	if ((ipsec_acc_enable==false) || (!(x->type->flags&XFRM_TYPE_HW_ACC)))
		return esp6_output(x, skb);
#endif
	err = ipsec_esp_preProcess(x, skb, HASH_DIGEST_OUT);
	if (err < 0)
	{
...
int ipsec_hw_acc_supported(struct xfrm_state *x)
{
	char nameString[32];
	char hashKeyName[32];
	...
	if (strcmp(hashKeyName, HASH_MD5_HMAC) != 0)
	{
		if (strcmp(hashKeyName, HASH_SHA1_HMAC) != 0)
		{
			if (strcmp(hashKeyName, HASH_SHA256_HMAC) != 0)
			{		
				if (strcmp(hashKeyName, HASH_NULL_HMAC) != 0)
					return 0;
			}	
		}
	}
	...
	if(strcmp(nameString, CIPHER_DES_CBC) != 0)
	{
		if(strcmp(nameString, CIPHER_3DES_CBC) != 0)
		{
			if(strcmp(nameString, CIPHER_AES_CBC) != 0)
			{
				if(strcmp(nameString, CIPHER_NULL_ECB) != 0)
					return 0;
			}
		}
	}	
