...
static void ipmr_expire_process(unsigned long arg)
{
	...
	now = jiffies;
	expires = CONFIG_IP_MROUTE_EXPIRES_DS * HZ / 10; //dlink
...
static struct mfc_cache *ipmr_cache_alloc_unres(void)
{
	...
	if (c) {
		...
		c->mfc_un.unres.expires = jiffies + CONFIG_IP_MROUTE_EXPIRES_DS * HZ / 10; //dlink
	}
	return c;
}
...
ipmr_cache_unresolved(struct mr_table *mrt, vifi_t vifi, struct sk_buff *skb)
{
	...
	if (!found) {
        ...
        if (atomic_read(&mrt->cache_resolve_queue_len)>=CONFIG_IP_MROUTE_UNRESOLVED_SIZE || /* dlink */
		    (c = ipmr_cache_alloc_unres()) == NULL) {
			spin_unlock_bh(&mfc_unres_lock);

			kfree_skb(skb);
            //printk("******** ENOBUFS: cache: %u, c = %p\n",atomic_read(&mrt->cache_resolve_queue_len),c);
			return -ENOBUFS;
		}
...
{
	int err;
	printk(KERN_NOTICE "multicast routing: unresolved cache size is %d, expires at %d.%d sec (HZ = %u)\n",
		CONFIG_IP_MROUTE_UNRESOLVED_SIZE,
		CONFIG_IP_MROUTE_EXPIRES_DS / 10,
		CONFIG_IP_MROUTE_EXPIRES_DS % 10,
		HZ);
