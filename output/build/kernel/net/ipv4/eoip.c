...
struct eoip_priv {
	...
	unsigned long keepalive_retries_proc;
	unsigned long keepalive_interval_proc;
	unsigned long keepalive_retries;
	unsigned long keepalive_interval;
};

static const struct ctl_table eoip_param_table[] = {
	{
		.procname      = "keepalive_interval_secs",
		.maxlen        = sizeof(unsigned long),
		.mode          = 0644,
		.proc_handler  = proc_doulongvec_minmax,
	},
	{
		.procname      = "keepalive_retries",
		.maxlen        = sizeof(unsigned long),
		.mode          = 0644,
		.proc_handler  = proc_doulongvec_minmax,
	},
...
static void eoip_unregister_dev_sysctl(struct eoip_priv *priv)
{
	struct ctl_table_header *header = priv->sysheader;
...
{
	struct eoip_priv *priv = netdev_priv(dev);

	eoip_unregister_dev_sysctl(priv);
...
{
	struct eoip_priv *priv = netdev_priv(dev);
	struct ip_tunnel *tunnel = &priv->tunnel;
...
static inline void *from_work(struct work_struct *work)
{
	struct delayed_work *delayed_work;
	struct eoip_priv *priv;

	delayed_work = container_of(work, struct delayed_work, work);
	priv = container_of(delayed_work, struct eoip_priv, keepalive_work);

	return priv;
}

static inline void send_keepalive(struct net_device *dev)
{
	struct sk_buff *skb;

	skb = alloc_skb(42, GFP_KERNEL);
	skb_reserve(skb, 42);
	skb->dev = dev;

	eoip_if_xmit(skb, dev);
}

static void keepalive_event(struct work_struct *work)
{
	struct eoip_priv *priv = from_work(work);

	if (time_before(priv->last_xmit + priv->keepalive_interval - 1, jiffies)) {
		send_keepalive(priv->tunnel.dev);
	}

	if (time_before(priv->last_rcv + priv->keepalive_interval * priv->keepalive_retries, jiffies)) {
		netif_carrier_off(priv->tunnel.dev);
	}

	queue_delayed_work(priv->wq, &priv->keepalive_work, priv->keepalive_interval / 2);
}
...
static int eoip_register_dev_sysctl(struct eoip_priv *priv)
{
	char path[sizeof("net/ipv4/conf/") + IFNAMSIZ];

	struct ctl_table *table;

	table = kmemdup(eoip_param_table, sizeof(eoip_param_table), GFP_KERNEL);
	if (!table) {
		return -ENOMEM;
	}

	snprintf(path, sizeof(path), "net/ipv4/conf/%s", priv->tunnel.dev->name);

	table[0].data = &priv->keepalive_interval_proc;
	table[1].data = &priv->keepalive_retries_proc;

	priv->sysheader = register_net_sysctl(dev_net(priv->tunnel.dev), path, table);
	if (!priv->sysheader) {
		kfree(table);
		return -ENOMEM;
	}
...
static int eoip_if_init(struct net_device *dev)
{
	...
	priv->keepalive_interval_proc = 0;
	priv->keepalive_retries_proc = 0;

	INIT_DELAYED_WORK(&priv->keepalive_work, keepalive_event);
	eoip_register_dev_sysctl(priv);

	priv->wq = create_singlethread_workqueue(dev->name);
	if (!priv->wq)
		return -ENOMEM;
...
static int eoip_open(struct net_device *dev)
{
	struct eoip_priv *priv = netdev_priv(dev);

	priv->keepalive_retries = priv->keepalive_retries_proc;
	priv->keepalive_interval = msecs_to_jiffies(priv->keepalive_interval_proc * 1000);
	priv->last_rcv = jiffies;
	priv->last_xmit = jiffies;

	if (priv->keepalive_interval && priv->keepalive_retries) {
		send_keepalive(dev);
		queue_delayed_work(priv->wq, &priv->keepalive_work, priv->keepalive_interval);
	}

	return 0;
}

static int eoip_stop(struct net_device *dev)
{
	struct eoip_priv *priv = netdev_priv(dev);
...
static struct rtnl_link_ops eoip_ops __read_mostly = {
	...
	.policy		= eoip_policy,
	.priv_size	= sizeof(struct eoip_priv),
	.setup		= eoip_setup,
