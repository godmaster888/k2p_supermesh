...
extern int 
ipsec_hw_acc_supported(
	struct xfrm_state *x
);
extern bool ipsec_acc_enable;
#endif
...
static
#endif
int esp6_output(struct xfrm_state *x, struct sk_buff *skb)
{
	int err;
...
#if defined (CONFIG_RALINK_HWCRYPTO) || defined (CONFIG_RALINK_HWCRYPTO_MODULE)
	if (ipsec_hw_acc_supported(x) && (ipsec_acc_enable==true))
		((struct xfrm_type*)(x->type))->flags |= XFRM_TYPE_HW_ACC;
#endif
