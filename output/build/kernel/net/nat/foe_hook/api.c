...
int ra_is_lan(uint16_t vid)
{
#if defined(CONFIG_DRU_RA_VLANS)
	size_t i;
	for (i = 0; i < ARRAY_SIZE(dru_sysctl_data.ra_vlans.lan_vid); i++) {
		uint16_t x = (uint16_t)dru_sysctl_data.ra_vlans.lan_vid[i];
		if (x && (vid == x))
			return 1;
	}
	return 0;
...
EXPORT_SYMBOL(ra_is_lan);

int ra_is_lan_be(__be16 vid_be)
{
#if defined(CONFIG_DRU_RA_VLANS)
	size_t i;
	for (i = 0; i < ARRAY_SIZE(dru_sysctl_data.ra_vlans.lan_vid_be16); i++) {
		__be16 x = dru_sysctl_data.ra_vlans.lan_vid_be16[i];
		if (x && (vid_be == x))
			return 1;
	}
	return 0;
#else
	return (vid_be == htons(lan_vid));
#endif
}
EXPORT_SYMBOL(ra_is_lan_be);
