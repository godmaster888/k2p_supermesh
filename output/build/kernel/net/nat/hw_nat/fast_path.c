...
//#include "fast_path.h" farisey: moved below ra_nat.h to get PktParseResult
...
//dlink, dkavanin
#if defined(CONFIG_GE_RGMII_INTERNAL_P4_AN) || defined(CONFIG_GE_RGMII_INTERNAL_P0_AN)
#define add_VLAN_LEN(prot, len) (len += (prot == htons(ETH_P_8021Q)) ? sizeof(struct vlan_hdr) : 0)
#else
#define add_VLAN_LEN(prot, len) {}
#endif
...
{		
...
#if defined (CONFIG_HNAT_V2) && (defined (CONFIG_RALINK_MT7620) || defined (CONFIG_RALINK_MT7621))
	...
	int vlan_len = VLAN_LEN;
	add_VLAN_LEN(((struct ethhdr *) LAYER2_HEADER(skb))->h_proto, vlan_len);

	iph = (struct iphdr *)(skb->data + vlan_len);
	if(iph->protocol != IPPROTO_GRE || skb->len < 60 + (vlan_len - VLAN_LEN)) {
	return 1;
    }
	...
	// #31759 dkavanin: китайцы почему-то решили, что если поток забинден, то обновлять адреса не нужно ни для кого,
	// и не важно, что на туннеле адрес уже сменился. Может в этом какой-то скрытый смысл... время покажет.
	// Нигде нет обнуления fast_bind, только при загрузке модуля.
	//if(!fast_bind){
	...
	if(iph_ppp0->protocol == IPPROTO_TCP){
		...
		if (HnatDebugLevel >= 1) {
	    //farisey disabled below to avoid log flooding
...
int32_t HnatPptpWan(struct sk_buff *skb)
{
...
#if defined(CONFIG_GE_RGMII_INTERNAL_P4_AN) || defined(CONFIG_GE_RGMII_INTERNAL_P0_AN)
	unsigned int vlan_tci = 0x1ffff;

	if (((struct ethhdr *) LAYER2_HEADER(skb))->h_proto == htons(ETH_P_8021Q)) {

		vlan_len += sizeof(struct vlan_hdr);
		vlan_tci = ((struct vlan_hdr *) (LAYER2_HEADER(skb) + sizeof(struct ethhdr)))->h_vlan_TCI;
	}
#endif
	...
	add_len = ETH_HLEN + VLAN_LEN + sizeof(struct iphdr) + sizeof(struct pptp_gre_header); // -4; // затирает первые 4 байта ip заголовка
#else
...
{		
	...
	iph = (struct iphdr *)(skb->data + vlan_len);
	if (iph->protocol != IPPROTO_UDP || skb->len < 60 + vlan_len) {
	return 1;
    }
...
int32_t  HnatPptpL2tpInit(void)
{
	...
	// dkavanin: можно передавать имя вана через параметр, но пока, думаю, не стоит. на данный момент всего два варианта: eth3 и wan1
    if (WanInf == NULL)
		WanInf = ra_dev_get_by_name("wan1");
...
int32_t SendL2TPHashPkt(struct sk_buff *pskb)
{
	...
	0x00, 0x00, 0x0a, 0x0a, 0x1e, 0x0a, 0x0a, 0x0a, 0x1e, 0x0b,//dlink: empty cksum
...
#ifdef CONFIG_RAETH_GMAC2
	iph = (struct iphdr *)(pskb->data+14 + VLAN_LEN + 4); // dkavanin: +4
#else
	...
	if(iph->protocol == IPPROTO_TCP){
		...
		memcpy(skb->data+14+4+sizeof(struct iphdr)+sizeof(struct udphdr), pskb->data, (14+VLAN_LEN+sizeof(struct iphdr)+sizeof(struct udphdr)));
		...
		if (HnatDebugLevel >= 1) {
			printk("Non TCP/UDP iph->protocol == 0x%x\n", iph->protocol);
		}
	}
...
int32_t PptpToLanParseLayerInfo(struct sk_buff * skb, PktParseResult *PpeParseResult)
{
	...
	//else if(iph_ori->protocol == IPPROTO_TCP){ dlink: bug!! UPDATE: fixed in 5.0.1.0
...
int32_t PptpToWanParseLayerInfo(struct sk_buff * skb, PktParseResult *PpeParseResult)
{
    ...
    eth = (struct ethhdr *)(skb->data);
	add_VLAN_LEN(eth->h_proto, vlan_len);
	iph_ppp0 = (struct iphdr *)(skb->data + 14 + vlan_len); // dkavanin: replaced 4 with VLAN_LEN

    memcpy(PpeParseResult->smac, eth->h_dest, ETH_ALEN);
    memcpy(PpeParseResult->dmac, eth->h_source, ETH_ALEN);
    PpeParseResult->smac[0] = 0x01;

	greh = (struct pptp_gre_header *)(skb->data + 14 + (iph_ppp0->ihl*4) + vlan_len);
