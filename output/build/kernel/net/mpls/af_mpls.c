...
{
	struct mpls_dev *mdev = mpls_dev_get(dev);

	return mdev && mdev->input_enabled;
}
EXPORT_SYMBOL_GPL(mpls_input_possible_rcu);
...
static bool egress_lo_hack = true;
module_param(egress_lo_hack, bool, 0644);
MODULE_PARM_DESC(egress_lo_hack, "Receive egress on orig netdev instead of lo");
			...
			struct packet_type *pt, struct net_device *orig_dev)
{
	...
	if ((skb->pkt_type == PACKET_OUTGOING) && (dev == net->loopback_dev)) {
		/* outgoing from vplsX or any, process as a usual forward. */
	}
	...
	if ((skb->pkt_type == PACKET_HOST) && (dev == net->loopback_dev)) {
		/* unpacked (w/o explicit null) landed on loop */
		err = mpls_substack_rcv(net, skb, &dec);
		switch (err) {
			case NOTIFY_DONE:
				break;
	...
	if ((skb->pkt_type == PACKET_OUTGOING) && (out_dev == dev))
		goto tx_err;
	...
	if (unlikely(!new_header_size && dec.bos)) {
		bool do_hack = egress_lo_hack && (out_dev == net->loopback_dev);
		/* Penultimate hop popping */
		if (do_hack)
			skb->dev = dev; /* receive on primary netdev */
		if (!mpls_egress(dev_net(skb->dev), rt, skb, dec))
			goto err;
		if (do_hack)
			return netif_rx(skb);
	} else {
...
static ATOMIC_NOTIFIER_HEAD(mpls_chain);

int register_mpls_notifier(struct notifier_block *nb)
{
        return atomic_notifier_chain_register(&mpls_chain, nb);
}
EXPORT_SYMBOL_GPL(register_mpls_notifier);

int unregister_mpls_notifier(struct notifier_block *nb)
{
        return atomic_notifier_chain_unregister(&mpls_chain, nb);
}
EXPORT_SYMBOL_GPL(unregister_mpls_notifier);

static int mpls_notifier_call_chain(unsigned long val,
				    struct mpls_notifier_data *v)
{
        return atomic_notifier_call_chain(&mpls_chain, val, v);
}
			      ...
			      const struct nl_info *info)
{
	...
	if (rt && (index >= MPLS_LABEL_FIRST_UNRESERVED)) {
		struct mpls_notifier_data n = {
			.net = net,
			.label = index,
			.replaced = !!nlm_flags,
			.fec = rt->rt_fec
		};
		mpls_notifier_call_chain((unsigned long)event, &n);
...
static int mpls_route_add(struct mpls_route_config *cfg)
{
	...
	memcpy(&rt->rt_fec, &cfg->rc_fec, sizeof(cfg->rc_fec));
			       ...
			       struct mpls_route_config *cfg)
{
	...
	for (index = 0; index <= RTA_MAX; index++) {
		...
		case RTA_GATEWAY:
		{
			if (nla_len(nla) == sizeof(__be32)) {
				cfg->rc_fec.fec_family = AF_INET;
				cfg->rc_fec.ipv4.s_addr = nla_get_in_addr(nla);
#if IS_ENABLED(CONFIG_IPV6)
			} else if (nla_len(nla) == sizeof(struct in6_addr)) {
				cfg->rc_fec.fec_family = AF_INET6;
				cfg->rc_fec.ipv6 = nla_get_in6_addr(nla);
#endif
			} else {
				goto errout;
			}
		...
		case RTA_PROTOINFO:
		{
			cfg->rc_fec.fec_prefixlen = nla_get_u8(nla);
			break;
		}
		default:
	...
	if (cfg->rc_fec.fec_family == AF_INET) {
		if (cfg->rc_fec.fec_prefixlen > 32)
			goto errout;
	} else if (cfg->rc_fec.fec_family == AF_INET6) {
		if (cfg->rc_fec.fec_prefixlen > 128)
			goto errout;
	}
			   ...
			   u32 label, struct mpls_route *rt, int flags)
{
	...
	if (rt->rt_fec.fec_family == AF_INET) {
		if (nla_put_in_addr(skb, RTA_GATEWAY, rt->rt_fec.ipv4.s_addr))
			goto nla_put_failure;
#if IS_ENABLED(CONFIG_IPV6)
	} else if (rt->rt_fec.fec_family == AF_INET6) {
		if (nla_put_in6_addr(skb, RTA_GATEWAY, &rt->rt_fec.ipv6))
			goto nla_put_failure;
#endif
	}
	if (rt->rt_fec.fec_family != AF_UNSPEC) {
		if (nla_put_u8(skb, RTA_PROTOINFO, rt->rt_fec.fec_prefixlen))
			goto nla_put_failure;
	}
...
static bool addr_matches_net(int af, const void *addr,
			     const void *net, u8 prefixlen)
{
	if (af == AF_INET) {
		__be32 mask = prefixlen ? htonl(~((1UL << (32 - prefixlen)) - 1)) : 0;
		__be32 a = ((struct in_addr*)addr)->s_addr;
		__be32 b = ((struct in_addr*)net)->s_addr;
		//truth table: 0 0 0 1 0 1 0 0
		return !((a^b) & mask);
		//return (a & mask) == (b & mask);
#if IS_ENABLED(CONFIG_IPV6)
	} else if (af == AF_INET6) {
		struct in6_addr a, b;
		ipv6_addr_prefix(&a, addr, prefixlen);
		ipv6_addr_prefix(&b, net, prefixlen);
...
{
	if (af != (int)fec->fec_family)
		return false;

	return addr_matches_net(af, addr, &fec->fec_addr, fec->fec_prefixlen);
}
EXPORT_SYMBOL_GPL(mpls_fec_matches);
...
int mpls_label_find_fec(struct net *net, int af, const void *addr)
{
	...
	for (index = MPLS_LABEL_FIRST_UNRESERVED; index < platform_labels; index++) {
		...
		if (mpls_fec_matches(af, addr, &rt->rt_fec))
			return index;
	}

	return 0;
}
EXPORT_SYMBOL_GPL(mpls_label_find_fec);
...
static struct rtnl_af_ops mpls_af_ops __read_mostly = {
	.family		   = AF_MPLS,
#if 0
//it was introduced in v4.10-rc3-659-gaefb4d4ad83b only
//Too huge API mismatch to be backported onto 3.10.108.
